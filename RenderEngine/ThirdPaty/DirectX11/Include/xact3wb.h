...
//
// Table for converting WMA Average Bytes per Second values to the
// WAVEBANKMINIWAVEFORMAT wBlockAlign field NOTE: There can be a max of 8 values
// in the table.
//

#define MAX_WMA_AVG_BYTES_PER_SEC_ENTRIES 7

static const DWORD aWMAAvgBytesPerSec[] = {12000, 24000, 4000, 6000,
                                           8000,  20000, 2500};
// bitrate = entry * 8

//
// Table for converting WMA Block Align values to the WAVEBANKMINIWAVEFORMAT
// wBlockAlign field NOTE: There can be a max of 32 values in the table.
//

#define MAX_WMA_BLOCK_ALIGN_ENTRIES 17

static const DWORD aWMABlockAlign[] = {929,  1487, 1280, 2230, 8917, 8192,
                                       4459, 5945, 2304, 1536, 1485, 1008,
...
typedef union WAVEBANKMINIWAVEFORMAT {
  struct {
    ...
    DWORD wBlockAlign : 8;     // Block alignment.  For WMA, lower 6 bits block
                               // alignment index, upper 2 bits bytes-per-second
                               // index.
    DWORD wBitsPerSample : 1;  // Bits per sample (8 vs. 16, PCM only);
                               // WMAudio2/WMAudio3 (for WMA)
  };
  ...
  WORD BitsPerSample() const {
    ...
    // wFormatTag must be WAVEBANKMINIFORMAT_TAG_PCM (2 bits can only represent
    // 4 different values)
...
#define ADPCM_MINIWAVEFORMAT_BLOCKALIGN_CONVERSION_OFFSET 22
  DWORD BlockAlign() const {
    ...
    case WAVEBANKMINIFORMAT_TAG_ADPCM:
      dwReturn =
          (wBlockAlign + ADPCM_MINIWAVEFORMAT_BLOCKALIGN_CONVERSION_OFFSET) *
          nChannels;
      break;
  ...
  DWORD EncodeWMABlockAlign(DWORD dwBlockAlign, DWORD dwAvgBytesPerSec) const {
    ...
    for (; dwBlockAlignIndex < MAX_WMA_BLOCK_ALIGN_ENTRIES &&
           dwBlockAlign != aWMABlockAlign[dwBlockAlignIndex];
         dwBlockAlignIndex++)
      ;
  ...
  void XMA2FillFormatEx(XMA2WAVEFORMATEX *fmt, WORD blockCount,
                        const struct WAVEBANKENTRY *entry) const;
  ...
  void AdpcmFillCoefficientTable(ADPCMWAVEFORMAT *fmt) const {
    ...
    static ADPCMCOEFSET aCoef[7] = {{256, 0},   {512, -256}, {0, 0},
...
inline void
WAVEBANKMINIWAVEFORMAT::XMA2FillFormatEx(XMA2WAVEFORMATEX *fmt, WORD blockCount,
                                         const WAVEBANKENTRY *entry) const {
  // Note caller is responsbile for filling out fmt->wfx with other helper
  // functions.
