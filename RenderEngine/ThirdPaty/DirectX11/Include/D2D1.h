...
//
//  Synopsis:
//      Qualifies how alpha is to be treated in a bitmap or render target
//      containing alpha.
//
//------------------------------------------------------------------------------
typedef enum D2D1_ALPHA_MODE {
  ...
  // supply or imply this information in which case alpha must be specified.
  //
...
typedef enum D2D1_EXTEND_MODE {
  ...
  //
  // Extend the edges of the source out by clamping sample points outside the
  // source to the edges.
  //
  D2D1_EXTEND_MODE_CLAMP = 0,

  //
  // The base tile is drawn untransformed and the remainder are filled by
  // repeating the base tile.
  //
...
//
//  Synopsis:
//      Enum which descibes the manner in which we render edges of non-text
//      primitives.
//
//------------------------------------------------------------------------------
...
//
//  Synopsis:
//      This enumeration describes the type of combine operation to be
//      performed.
//
//------------------------------------------------------------------------------
...
typedef enum D2D1_GEOMETRY_RELATION {
  ...
  //
  // The relation between the geometries couldn't be determined. This value is
  // never returned by any D2D method.
  //
...
//
//  Synopsis:
//      Indicates whether the given segment should be stroked, or, if the join
//      between this segment and the previous one should be smooth.
//
//------------------------------------------------------------------------------
...
//
//  Synopsis:
//      Properties, aside from the width, that allow geometric penning to be
//      specified.
//
//------------------------------------------------------------------------------
...
//
//  Synopsis:
//      Specified options that can be applied when a layer resource is applied
//      to create a layer.
//
//------------------------------------------------------------------------------
typedef enum D2D1_LAYER_OPTIONS {
  ...
  //
  // The layer will render correctly for ClearType text. If the render target
  // was set to ClearType previously, the layer will continue to render
  // ClearType. If the render target was set to ClearType and this option is not
  // specified, the render target will be set to render gray-scale until the
  // layer is popped. The caller can override this default by calling
  // SetTextAntialiasMode while within the layer. This flag is slightly slower
  // than the default.
  //
...
typedef struct D2D1_LAYER_PARAMETERS {
  ...
  //
  // The rectangular clip that will be applied to the layer. The clip is
  // affected by the world transform. Content outside of the content bounds will
  // not render.
  //
  ...
  // inside the fill of the mask will not be rendered.
  //
  ...
  //
  // The opacity with which all of the content in the layer will be blended back
  // to the target when the layer is popped.
  //
  FLOAT opacity;

  //
  // An additional brush that can be applied to the layer. Only the opacity
  // channel is sampled from this brush and multiplied both with the layer
  // content and the over-all layer opacity.
  //
...
typedef enum D2D1_RENDER_TARGET_USAGE {
  ...
  //
  // Rendering will occur locally, if a terminal-services session is
  // established, the bitmap updates will be sent to the terminal services
  // client.
  //
...
//
//  Synopsis:
//      Allows the drawing state to be atomically created. This also specifies
//      the drawing state that is saved into an IDrawingStateBlock object.
//
//------------------------------------------------------------------------------
...
//
//  Synopsis:
//      Specifies the threading model of the created factory and all of its
//      derived resources.
//
//------------------------------------------------------------------------------
typedef enum D2D1_FACTORY_TYPE {
  ...
  //
  // The resulting factory may be invoked from multiple threads. Returned
  // resources use interlocked reference counting and their state is protected.
  //
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd90691-12e2-11dc-9fed-001143a055f9")
    ID2D1Resource : public IUnknown {

  //
  // Retrieve the factory associated with this resource.
  //
  STDMETHOD_(void, GetFactory)(__deref_out ID2D1Factory * *factory) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("a2296057-ea42-4099-983b-539fb6505426")
    ID2D1Bitmap : public ID2D1Resource {
  ...
  //
  STDMETHOD_(void, GetDpi)(__out FLOAT * dpiX, __out FLOAT * dpiY) CONST PURE;

  STDMETHOD(CopyFromBitmap)
  (__in_opt CONST D2D1_POINT_2U * destPoint, __in ID2D1Bitmap * bitmap,
   __in_opt CONST D2D1_RECT_U * srcRect) PURE;

  STDMETHOD(CopyFromRenderTarget)
  (__in_opt CONST D2D1_POINT_2U * destPoint,
   __in ID2D1RenderTarget * renderTarget, __in_opt CONST D2D1_RECT_U * srcRect)
      PURE;

  STDMETHOD(CopyFromMemory)
  (__in_opt CONST D2D1_RECT_U * dstRect, __in CONST void *srcData, UINT32 pitch)
      PURE;
...
//
//  Synopsis:
//      Represents an collection of gradient stops that can then be the source
//      resource for either a linear or radial gradient brush.
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a7-12e2-11dc-9fed-001143a055f9")
  ...
  (__out_ecount(gradientStopsCount) D2D1_GRADIENT_STOP * gradientStops,
   UINT gradientStopsCount) CONST PURE;
...
//
//  Synopsis:
//      The root brush interface. All brushes can be used to fill or pen a
//      geometry.
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a8-12e2-11dc-9fed-001143a055f9")
  ...
  //
  STDMETHOD_(void, SetTransform)(__in CONST D2D1_MATRIX_3X2_F * transform) PURE;

  STDMETHOD_(FLOAT, GetOpacity)() CONST PURE;

  STDMETHOD_(void, GetTransform)
  (__out D2D1_MATRIX_3X2_F * transform) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906aa-12e2-11dc-9fed-001143a055f9")
  ...
  (D2D1_BITMAP_INTERPOLATION_MODE interpolationMode) PURE;

  //
  // Sets the bitmap associated as the source of this brush.
  //
  STDMETHOD_(void, SetBitmap)(__in ID2D1Bitmap * bitmap) PURE;
  ...
  STDMETHOD_(void, GetBitmap)(__deref_out ID2D1Bitmap * *bitmap) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a9-12e2-11dc-9fed-001143a055f9")
    ID2D1SolidColorBrush : public ID2D1Brush {

  STDMETHOD_(void, SetColor)(__in CONST D2D1_COLOR_F * color) PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906ab-12e2-11dc-9fed-001143a055f9")
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906ac-12e2-11dc-9fed-001143a055f9")
    ID2D1RadialGradientBrush : public ID2D1Brush {

  //
  // Sets the center of the radial gradient. This will be in local coordinates
  // and will not depend on the geometry being filled.
  //
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd9069d-12e2-11dc-9fed-001143a055f9")
  ...
  // must call GetDashesCount to retrieve the required size.
  //
  STDMETHOD_(void, GetDashes)
  (__out_ecount(dashesCount) FLOAT * dashes, UINT dashesCount) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a1-12e2-11dc-9fed-001143a055f9")
  ...
  (__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
   __out D2D1_RECT_F * bounds) CONST PURE;

  //
  // Get the bounds of the corresponding geometry after it has been widened or
  // have an optional pen style applied.
  //
  STDMETHOD(GetWidenedBounds)
  (FLOAT strokeWidth, __in_opt ID2D1StrokeStyle * strokeStyle,
   __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform, FLOAT flatteningTolerance,
   __out D2D1_RECT_F * bounds) CONST PURE;

  //
  // Checks to see whether the corresponding penned and widened geometry
  ...
  (D2D1_POINT_2F point, FLOAT strokeWidth,
   ...
   __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform, FLOAT flatteningTolerance,
   __out BOOL * contains) CONST PURE;

  //
  // Test whether the given fill of this geometry would contain this point.
  //
  STDMETHOD(FillContainsPoint)
  (D2D1_POINT_2F point, __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
   FLOAT flatteningTolerance, __out BOOL * contains) CONST PURE;
  ...
  (__in ID2D1Geometry * inputGeometry,
   ...
   FLOAT flatteningTolerance, __out D2D1_GEOMETRY_RELATION * relation)
      CONST PURE;
  ...
  (D2D1_GEOMETRY_SIMPLIFICATION_OPTION simplificationOption,
   __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform, FLOAT flatteningTolerance,
   __in ID2D1SimplifiedGeometrySink * geometrySink) CONST PURE;
  ...
  (__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform, FLOAT flatteningTolerance,
   __in ID2D1TessellationSink * tessellationSink) CONST PURE;

  //
  // Performs a combine operation between the two geometries to produce a
  // resulting geometry.
  //
  STDMETHOD(CombineWithGeometry)
  (__in ID2D1Geometry * inputGeometry, D2D1_COMBINE_MODE combineMode,
   __in_opt CONST D2D1_MATRIX_3X2_F * inputGeometryTransform,
   FLOAT flatteningTolerance, __in ID2D1SimplifiedGeometrySink * geometrySink)
      CONST PURE;
  ...
  (__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform, FLOAT flatteningTolerance,
   __out FLOAT * length) CONST PURE;

  //
  // Computes the point and tangent a given distance along the path.
  //
  STDMETHOD(ComputePointAtLength)
  (FLOAT length, __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
   FLOAT flatteningTolerance, __out_opt D2D1_POINT_2F * point,
   __out_opt D2D1_POINT_2F * unitTangentVector) CONST PURE;
  ...
  //
  HRESULT
  GetBounds(CONST D2D1_MATRIX_3X2_F & worldTransform,
            __out D2D1_RECT_F * bounds) CONST {
  ...
  //
  HRESULT
  GetWidenedBounds(FLOAT strokeWidth, __in_opt ID2D1StrokeStyle * strokeStyle,
                   CONST D2D1_MATRIX_3X2_F & worldTransform,
                   FLOAT flatteningTolerance, __out D2D1_RECT_F * bounds)
      CONST {
    return GetWidenedBounds(strokeWidth, strokeStyle, &worldTransform,
  ...
  GetWidenedBounds(FLOAT strokeWidth, __in_opt ID2D1StrokeStyle * strokeStyle,
                   ...
                   __out D2D1_RECT_F * bounds) CONST {
    return GetWidenedBounds(strokeWidth, strokeStyle, worldTransform,
                            D2D1_DEFAULT_FLATTENING_TOLERANCE, bounds);
  }
  ...
  HRESULT
  StrokeContainsPoint(D2D1_POINT_2F point, FLOAT strokeWidth,
                      __in_opt ID2D1StrokeStyle * strokeStyle,
                      CONST D2D1_MATRIX_3X2_F & worldTransform,
                      FLOAT flatteningTolerance, __out BOOL * contains) CONST {
  ...
  HRESULT
  FillContainsPoint(D2D1_POINT_2F point,
                    CONST D2D1_MATRIX_3X2_F & worldTransform,
                    FLOAT flatteningTolerance, __out BOOL * contains) CONST {
  ...
  FillContainsPoint(D2D1_POINT_2F point,
                    __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
                    __out BOOL * contains) CONST {
    return FillContainsPoint(point, worldTransform,
                             D2D1_DEFAULT_FLATTENING_TOLERANCE, contains);
  }
  ...
  //
  HRESULT
  CompareWithGeometry(__in ID2D1Geometry * inputGeometry,
                      CONST D2D1_MATRIX_3X2_F & inputGeometryTransform,
                      FLOAT flatteningTolerance,
                      __out D2D1_GEOMETRY_RELATION * relation) CONST {
    return CompareWithGeometry(inputGeometry, &inputGeometryTransform,
  ...
  CompareWithGeometry(__in ID2D1Geometry * inputGeometry,
                      ...
                      __out D2D1_GEOMETRY_RELATION * relation) CONST {
    return CompareWithGeometry(inputGeometry, inputGeometryTransform,
                               D2D1_DEFAULT_FLATTENING_TOLERANCE, relation);
  }
  ...
  //
  HRESULT
  Simplify(D2D1_GEOMETRY_SIMPLIFICATION_OPTION simplificationOption,
           CONST D2D1_MATRIX_3X2_F & worldTransform, FLOAT flatteningTolerance,
           __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return Simplify(simplificationOption, &worldTransform, flatteningTolerance,
                    geometrySink);
  }
  ...
  Simplify(D2D1_GEOMETRY_SIMPLIFICATION_OPTION simplificationOption,
           __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
           __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return Simplify(simplificationOption, worldTransform,
                    D2D1_DEFAULT_FLATTENING_TOLERANCE, geometrySink);
  }
  ...
  //
  HRESULT
  Tessellate(CONST D2D1_MATRIX_3X2_F & worldTransform,
             FLOAT flatteningTolerance,
             __in ID2D1TessellationSink * tessellationSink) CONST {
  ...
  //
  HRESULT
  Tessellate(__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
             __in ID2D1TessellationSink * tessellationSink) CONST {
    return Tessellate(worldTransform, D2D1_DEFAULT_FLATTENING_TOLERANCE,
                      tessellationSink);
  }
  ...
  //
  HRESULT
  CombineWithGeometry(__in ID2D1Geometry * inputGeometry,
                      D2D1_COMBINE_MODE combineMode,
                      CONST D2D1_MATRIX_3X2_F & inputGeometryTransform,
                      FLOAT flatteningTolerance,
                      __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return CombineWithGeometry(inputGeometry, combineMode,
  ...
  //
  HRESULT
  Outline(CONST D2D1_MATRIX_3X2_F & worldTransform, FLOAT flatteningTolerance,
          __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
  ...
  //
  HRESULT
  Outline(__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
          __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return Outline(worldTransform, D2D1_DEFAULT_FLATTENING_TOLERANCE,
                   geometrySink);
  }
  ...
  //
  HRESULT
  Outline(CONST D2D1_MATRIX_3X2_F & worldTransform,
          __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return Outline(&worldTransform, D2D1_DEFAULT_FLATTENING_TOLERANCE,
                   geometrySink);
  }
  ...
  //
  HRESULT
  ComputeArea(CONST D2D1_MATRIX_3X2_F & worldTransform,
              FLOAT flatteningTolerance, __out FLOAT * area) CONST {
  ...
  //
  HRESULT
  ComputeArea(__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
              __out FLOAT * area) CONST {
  ...
  //
  HRESULT
  ComputeArea(CONST D2D1_MATRIX_3X2_F & worldTransform, __out FLOAT * area)
      CONST {
    return ComputeArea(&worldTransform, D2D1_DEFAULT_FLATTENING_TOLERANCE,
                       area);
  }
  ...
  //
  HRESULT
  ComputeLength(CONST D2D1_MATRIX_3X2_F & worldTransform,
                FLOAT flatteningTolerance, __out FLOAT * length) CONST {
  ...
  //
  HRESULT
  ComputeLength(__in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
                __out FLOAT * length) CONST {
  ...
  //
  HRESULT
  ComputeLength(CONST D2D1_MATRIX_3X2_F & worldTransform, __out FLOAT * length)
      CONST {
    return ComputeLength(&worldTransform, D2D1_DEFAULT_FLATTENING_TOLERANCE,
                         length);
  }
  ...
  //
  HRESULT
  ComputePointAtLength(FLOAT length, CONST D2D1_MATRIX_3X2_F & worldTransform,
                       FLOAT flatteningTolerance,
                       __out_opt D2D1_POINT_2F * point,
                       __out_opt D2D1_POINT_2F * unitTangentVector) CONST {
  ...
  //
  HRESULT
  ComputePointAtLength(FLOAT length,
                       __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
                       __out_opt D2D1_POINT_2F * point,
                       __out_opt D2D1_POINT_2F * unitTangentVector) CONST {
    return ComputePointAtLength(length, worldTransform,
                                D2D1_DEFAULT_FLATTENING_TOLERANCE, point,
                                unitTangentVector);
  }
  ...
  //
  HRESULT
  Widen(FLOAT strokeWidth, __in_opt ID2D1StrokeStyle * strokeStyle,
        CONST D2D1_MATRIX_3X2_F & worldTransform, FLOAT flatteningTolerance,
        __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return Widen(strokeWidth, strokeStyle, &worldTransform, flatteningTolerance,
                 geometrySink);
  }
  ...
  Widen(FLOAT strokeWidth, __in_opt ID2D1StrokeStyle * strokeStyle,
        __in_opt CONST D2D1_MATRIX_3X2_F * worldTransform,
        __in ID2D1SimplifiedGeometrySink * geometrySink) CONST {
    return Widen(strokeWidth, strokeStyle, worldTransform,
                 D2D1_DEFAULT_FLATTENING_TOLERANCE, geometrySink);
  }
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a2-12e2-11dc-9fed-001143a055f9")
    ID2D1RectangleGeometry : public ID2D1Geometry {

  STDMETHOD_(void, GetRect)(__out D2D1_RECT_F * rect) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a3-12e2-11dc-9fed-001143a055f9")
    ID2D1RoundedRectangleGeometry : public ID2D1Geometry {

  STDMETHOD_(void, GetRoundedRect)
  (__out D2D1_ROUNDED_RECT * roundedRect) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a4-12e2-11dc-9fed-001143a055f9")
    ID2D1EllipseGeometry : public ID2D1Geometry {

  STDMETHOD_(void, GetEllipse)(__out D2D1_ELLIPSE * ellipse) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a6-12e2-11dc-9fed-001143a055f9")
  ...
  (__out_ecount(geometriesCount) ID2D1Geometry * *geometries,
   UINT geometriesCount) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906bb-12e2-11dc-9fed-001143a055f9")
    ID2D1TransformedGeometry : public ID2D1Geometry {

  STDMETHOD_(void, GetSourceGeometry)
  (__deref_out ID2D1Geometry * *sourceGeometry) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd9069e-12e2-11dc-9fed-001143a055f9")
  ...
  (D2D1_POINT_2F startPoint, D2D1_FIGURE_BEGIN figureBegin) PURE;

  STDMETHOD_(void, AddLines)
  (__in_ecount(pointsCount) CONST D2D1_POINT_2F * points, UINT pointsCount)
      PURE;

  STDMETHOD_(void, AddBeziers)
  (__in_ecount(beziersCount) CONST D2D1_BEZIER_SEGMENT * beziers,
   UINT beziersCount) PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd9069f-12e2-11dc-9fed-001143a055f9")
    ID2D1GeometrySink : public ID2D1SimplifiedGeometrySink {

  STDMETHOD_(void, AddLine)(D2D1_POINT_2F point) PURE;

  STDMETHOD_(void, AddBezier)(__in CONST D2D1_BEZIER_SEGMENT * bezier) PURE;

  STDMETHOD_(void, AddQuadraticBezier)
  (__in CONST D2D1_QUADRATIC_BEZIER_SEGMENT * bezier) PURE;

  STDMETHOD_(void, AddQuadraticBeziers)
  (__in_ecount(beziersCount) CONST D2D1_QUADRATIC_BEZIER_SEGMENT * beziers,
   UINT beziersCount) PURE;

  STDMETHOD_(void, AddArc)(__in CONST D2D1_ARC_SEGMENT * arc) PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906c1-12e2-11dc-9fed-001143a055f9")
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd906a5-12e2-11dc-9fed-001143a055f9")
    ID2D1PathGeometry : public ID2D1Geometry {

  //
  // Opens a geometry sink that will be used to create this path geometry.
  //
  STDMETHOD(Open)(__deref_out ID2D1GeometrySink * *geometrySink) PURE;

  //
  // Retrieve the contents of this geometry. The caller passes an implementation
  // of a ID2D1GeometrySink interface to receive the data.
  //
  STDMETHOD(Stream)(__in ID2D1GeometrySink * geometrySink) CONST PURE;

  STDMETHOD(GetSegmentCount)(__out UINT32 * count) CONST PURE;

  STDMETHOD(GetFigureCount)(__out UINT32 * count) CONST PURE;
...
interface D2D1_DECLARE_INTERFACE(
    "2cd906c2-12e2-11dc-9fed-001143a055f9") ID2D1Mesh : public ID2D1Resource {

  //
  // Opens the mesh for population.
  //
  STDMETHOD(Open)(__deref_out ID2D1TessellationSink * *tessellationSink) PURE;
}; // interface ID2D1Mesh
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd9069b-12e2-11dc-9fed-001143a055f9")
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("28506e39-ebf6-46a1-bb47-fd85565ab957")
  ...
  (__out D2D1_DRAWING_STATE_DESCRIPTION * stateDescription) CONST PURE;

  //
  // Sets the state description of this state block resource.
  //
  STDMETHOD_(void, SetDescription)
  (__in CONST D2D1_DRAWING_STATE_DESCRIPTION * stateDescription) PURE;

  //
  // Sets the text rendering parameters of this state block resource.
  //
  STDMETHOD_(void, SetTextRenderingParams)
  (__in_opt IDWriteRenderingParams *textRenderingParams = NULL) PURE;
  ...
  (__deref_out_opt IDWriteRenderingParams * *textRenderingParams) CONST PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd90694-12e2-11dc-9fed-001143a055f9")
  ...
  (D2D1_SIZE_U size, __in_opt CONST void *srcData, UINT32 pitch,
   __in CONST D2D1_BITMAP_PROPERTIES *bitmapProperties,
   __deref_out ID2D1Bitmap **bitmap) PURE;
  ...
  //
  // Create a D2D bitmap by sharing bits from another resource. The bitmap must
  // be compatible with the render target for the call to succeed. For example,
  // an IWICBitmap can be shared with a software target, or a DXGI surface can
  // be shared with a DXGI render target.
  //
  STDMETHOD(CreateSharedBitmap)
  (__in REFIID riid, __inout void *data,
   __in_opt CONST D2D1_BITMAP_PROPERTIES *bitmapProperties,
   __deref_out ID2D1Bitmap **bitmap) PURE;

  //
  // Creates a bitmap brush. The bitmap is scaled, rotated, skewed or tiled to
  // fill or pen a geometry.
  //
  ...
  (__in ID2D1Bitmap * bitmap,
   ...
   __deref_out ID2D1BitmapBrush * *bitmapBrush) PURE;

  STDMETHOD(CreateSolidColorBrush)
  (__in CONST D2D1_COLOR_F * color,
   __in_opt CONST D2D1_BRUSH_PROPERTIES * brushProperties,
   __deref_out ID2D1SolidColorBrush * *solidColorBrush) PURE;

  //
  // A gradient stop collection represents a set of stops in an ideal unit
  // length. This is the source resource for a linear gradient and radial
  // gradient brush.
  //
  ...
  (__in_ecount(gradientStopsCount) CONST D2D1_GRADIENT_STOP * gradientStops,
   ...
   __deref_out ID2D1GradientStopCollection * *gradientStopCollection) PURE;

  STDMETHOD(CreateLinearGradientBrush)
  (__in CONST D2D1_LINEAR_GRADIENT_BRUSH_PROPERTIES *
       linearGradientBrushProperties,
   __in_opt CONST D2D1_BRUSH_PROPERTIES * brushProperties,
   __in ID2D1GradientStopCollection * gradientStopCollection,
   __deref_out ID2D1LinearGradientBrush * *linearGradientBrush) PURE;

  STDMETHOD(CreateRadialGradientBrush)
  (__in CONST D2D1_RADIAL_GRADIENT_BRUSH_PROPERTIES *
       radialGradientBrushProperties,
   __in_opt CONST D2D1_BRUSH_PROPERTIES * brushProperties,
   __in ID2D1GradientStopCollection * gradientStopCollection,
   __deref_out ID2D1RadialGradientBrush * *radialGradientBrush) PURE;
  ...
  (
      ...
      //
      // The requested size of the target in DIPs. If the pixel size is not
      // specified, the DPI is inherited from the parent target. However, the
      ...
      // also specified, the DPI is calculated from these two values. If the
      // desired size is not specified, the DPI is inherited from the parent
      // render target. If neither value is specified, the compatible render
      // target will be the same size and have the same DPI as the parent
      // target.
      //
      __in_opt CONST D2D1_SIZE_U * desiredPixelSize,

      //
      // The desired pixel format. The format must be compatible with the parent
      // render target type. If the format is not specified, it will be
      // inherited from the parent render target.
      //
      ...
      //
      __deref_out ID2D1BitmapRenderTarget * *bitmapRenderTarget) PURE;

  //
  // Creates a layer resource that can be used on any target and which will
  // resize under the covers if necessary.
  //
  ...
  (
      ...
      // Specify this to prevent unwanted reallocation of the layer backing
      // store. The size is in DIPs, but, it is unaffected by the current world
      // transform. If the size is unspecified, the returned resource is a
      // placeholder and the backing store will be allocated to be the minimum
      // size that can hold the content when the layer is pushed.
      //
      __in_opt CONST D2D1_SIZE_F * size, __deref_out ID2D1Layer * *layer) PURE;

  //
  // Create a D2D mesh.
  //
  STDMETHOD(CreateMesh)(__deref_out ID2D1Mesh * *mesh) PURE;

  STDMETHOD_(void, DrawLine)
  (D2D1_POINT_2F point0, D2D1_POINT_2F point1, __in ID2D1Brush * brush,
   FLOAT strokeWidth = 1.0f, __in_opt ID2D1StrokeStyle *strokeStyle = NULL)
      PURE;

  STDMETHOD_(void, DrawRectangle)
  (__in CONST D2D1_RECT_F * rect, __in ID2D1Brush * brush,
   FLOAT strokeWidth = 1.0f, __in_opt ID2D1StrokeStyle *strokeStyle = NULL)
      PURE;

  STDMETHOD_(void, FillRectangle)
  (__in CONST D2D1_RECT_F * rect, __in ID2D1Brush * brush) PURE;

  STDMETHOD_(void, DrawRoundedRectangle)
  (__in CONST D2D1_ROUNDED_RECT * roundedRect, __in ID2D1Brush * brush,
   FLOAT strokeWidth = 1.0f, __in_opt ID2D1StrokeStyle *strokeStyle = NULL)
      PURE;

  STDMETHOD_(void, FillRoundedRectangle)
  (__in CONST D2D1_ROUNDED_RECT * roundedRect, __in ID2D1Brush * brush) PURE;

  STDMETHOD_(void, DrawEllipse)
  (__in CONST D2D1_ELLIPSE * ellipse, __in ID2D1Brush * brush,
   FLOAT strokeWidth = 1.0f, __in_opt ID2D1StrokeStyle *strokeStyle = NULL)
      PURE;

  STDMETHOD_(void, FillEllipse)
  (__in CONST D2D1_ELLIPSE * ellipse, __in ID2D1Brush * brush) PURE;

  STDMETHOD_(void, DrawGeometry)
  (__in ID2D1Geometry * geometry, __in ID2D1Brush * brush,
   FLOAT strokeWidth = 1.0f, __in_opt ID2D1StrokeStyle *strokeStyle = NULL)
      PURE;
  ...
  (__in ID2D1Geometry * geometry, __in ID2D1Brush * brush,
   ...
   // corresponding brush will be sampled and will be applied to the entire fill
   // of the geometry. If this brush is specified, the fill brush must be a
   // bitmap brush with an extend mode of D2D1_EXTEND_MODE_CLAMP.
   //
   __in_opt ID2D1Brush *opacityBrush = NULL) PURE;

  //
  // Fill a mesh. Since meshes can only render aliased content, the render
  // target antialiasing mode must be set to aliased.
  //
  STDMETHOD_(void, FillMesh)
  (__in ID2D1Mesh * mesh, __in ID2D1Brush * brush) PURE;

  //
  // Fill using the opacity channel of the supplied bitmap as a mask. The alpha
  // channel of the bitmap is used to represent the coverage of the geometry at
  // each pixel, and this is filled appropriately with the brush. The render
  // target antialiasing mode must be set to aliased.
  //
  STDMETHOD_(void, FillOpacityMask)
  (__in ID2D1Bitmap * opacityMask, __in ID2D1Brush * brush,
   D2D1_OPACITY_MASK_CONTENT content,
   __in_opt CONST D2D1_RECT_F *destinationRectangle = NULL,
   __in_opt CONST D2D1_RECT_F *sourceRectangle = NULL) PURE;
  ...
  //
  // Draws the text within the given layout rectangle and by default also snaps
  // and clips it to the content bounds.
  //
  STDMETHOD_(void, DrawText)
  (__in_ecount(stringLength) CONST WCHAR * string, UINT stringLength,
   __in IDWriteTextFormat * textFormat, __in CONST D2D1_RECT_F * layoutRect,
   __in ID2D1Brush * defaultForegroundBrush,
   D2D1_DRAW_TEXT_OPTIONS options = D2D1_DRAW_TEXT_OPTIONS_NONE,
   DWRITE_MEASURING_MODE measuringMode = DWRITE_MEASURING_MODE_NATURAL) PURE;

  //
  // Draw a snapped text layout object. Since the layout is not subsequently
  // changed, this can be more effecient than DrawText when drawing the same
  // layout repeatedly.
  //
  STDMETHOD_(void, DrawTextLayout)
  (D2D1_POINT_2F origin, __in IDWriteTextLayout * textLayout,
   __in ID2D1Brush * defaultForegroundBrush,

   //
   // The specified text options. NOTE: By default the text is clipped to the
   // layout bounds. This is derived from the origin and the layout bounds of
   // the corresponding IDWriteTextLayout object.
   //
   D2D1_DRAW_TEXT_OPTIONS options = D2D1_DRAW_TEXT_OPTIONS_NONE) PURE;

  STDMETHOD_(void, DrawGlyphRun)
  (D2D1_POINT_2F baselineOrigin, __in CONST DWRITE_GLYPH_RUN * glyphRun,
  ...
  (D2D1_TEXT_ANTIALIAS_MODE textAntialiasMode) PURE;
  ...
  //
  // Retrieves the currently set tags. This does not retrieve the tags
  // corresponding to any primitive that is in error.
  //
  STDMETHOD_(void, GetTags)
  (__out_opt D2D1_TAG *tag1 = NULL, __out_opt D2D1_TAG *tag2 = NULL) CONST PURE;

  //
  // Start a layer of drawing calls. The way in which the layer must be resolved
  // is specified first as well as the logical resource that stores the layer
  ...
  (__in CONST D2D1_LAYER_PARAMETERS * layerParameters, __in ID2D1Layer * layer)
      PURE;
  ...
  STDMETHOD(Flush)
  (__out_opt D2D1_TAG *tag1 = NULL, __out_opt D2D1_TAG *tag2 = NULL) PURE;
  ...
  (__inout ID2D1DrawingStateBlock * drawingStateBlock) CONST PURE;

  //
  // Copies the state stored in the block interface.
  //
  STDMETHOD_(void, RestoreDrawingState)
  (__in ID2D1DrawingStateBlock * drawingStateBlock) PURE;

  //
  // Pushes a clip. The clip can be antialiased. The clip must be axis aligned.
  // If the current world transform is not axis preserving, then the bounding
  // box of the transformed clip rect will be used. The clip will remain in
  // effect until a PopAxisAligned clip call is made.
  //
  STDMETHOD_(void, PushAxisAlignedClip)
  (__in CONST D2D1_RECT_F * clipRect, D2D1_ANTIALIAS_MODE antialiasMode) PURE;

  STDMETHOD_(void, PopAxisAlignedClip)() PURE;

  STDMETHOD_(void, Clear)(__in_opt CONST D2D1_COLOR_F *clearColor = NULL) PURE;

  //
  // Start drawing on this render target. Draw calls can only be issued between
  // a BeginDraw and EndDraw call.
  //
  STDMETHOD_(void, BeginDraw)() PURE;

  //
  // Ends drawing on the render target, error results can be retrieved at this
  // time, or when calling flush.
  //
  STDMETHOD(EndDraw)
  (__out_opt D2D1_TAG *tag1 = NULL, __out_opt D2D1_TAG *tag2 = NULL) PURE;
  ...
  // The DPI is ignored. NOTE: If the render target type is software, then
  // neither D2D1_FEATURE_LEVEL_9 nor D2D1_FEATURE_LEVEL_10 will be considered
  // to be supported.
  //
  ...
  HRESULT
  CreateBitmap(D2D1_SIZE_U size, __in_opt CONST void *srcData, UINT32 pitch,
  ...
  //
  HRESULT
  CreateBitmapFromWicBitmap(__in IWICBitmapSource * wicBitmapSource,
                            CONST D2D1_BITMAP_PROPERTIES & bitmapProperties,
                            __deref_out ID2D1Bitmap * *bitmap) {
  ...
  //
  HRESULT
  CreateBitmapBrush(__in ID2D1Bitmap * bitmap,
  ...
  HRESULT
  CreateSolidColorBrush(CONST D2D1_COLOR_F & color,
  ...
  CreateGradientStopCollection(
      ...
      __deref_out ID2D1GradientStopCollection * *gradientStopCollection) {
    return CreateGradientStopCollection(gradientStops, gradientStopsCount,
  ...
  CreateLinearGradientBrush(
    ...
    return CreateLinearGradientBrush(&linearGradientBrushProperties, NULL,
                                     gradientStopCollection,
                                     linearGradientBrush);
  }
  ...
  CreateRadialGradientBrush(
    ...
    return CreateRadialGradientBrush(&radialGradientBrushProperties, NULL,
                                     gradientStopCollection,
                                     radialGradientBrush);
  }
  ...
  HRESULT
  CreateCompatibleRenderTarget(__deref_out ID2D1BitmapRenderTarget *
                               *bitmapRenderTarget) {
  ...
  HRESULT
  CreateCompatibleRenderTarget(D2D1_SIZE_F desiredSize,
                               __deref_out ID2D1BitmapRenderTarget *
                                   *bitmapRenderTarget) {
    return CreateCompatibleRenderTarget(
        &desiredSize, NULL, NULL, D2D1_COMPATIBLE_RENDER_TARGET_OPTIONS_NONE,
        bitmapRenderTarget);
  }

  HRESULT
  CreateCompatibleRenderTarget(
      D2D1_SIZE_F desiredSize, D2D1_SIZE_U desiredPixelSize,
      __deref_out ID2D1BitmapRenderTarget * *bitmapRenderTarget) {
    return CreateCompatibleRenderTarget(
        &desiredSize, &desiredPixelSize, NULL,
        D2D1_COMPATIBLE_RENDER_TARGET_OPTIONS_NONE, bitmapRenderTarget);
  }
  ...
  HRESULT
  CreateLayer(D2D1_SIZE_F size, __deref_out ID2D1Layer * *layer) {
    return CreateLayer(&size, layer);
  }

  HRESULT
  CreateLayer(__deref_out ID2D1Layer * *layer) {
    return CreateLayer(NULL, layer);
  }

  void DrawRectangle(CONST D2D1_RECT_F & rect, __in ID2D1Brush * brush,
                     FLOAT strokeWidth = 1.0f,
                     __in_opt ID2D1StrokeStyle *strokeStyle = NULL) {
    DrawRectangle(&rect, brush, strokeWidth, strokeStyle);
  }

  void FillRectangle(CONST D2D1_RECT_F & rect, __in ID2D1Brush * brush) {
    FillRectangle(&rect, brush);
  }

  void DrawRoundedRectangle(CONST D2D1_ROUNDED_RECT & roundedRect,
                            __in ID2D1Brush * brush, FLOAT strokeWidth = 1.0f,
                            __in_opt ID2D1StrokeStyle *strokeStyle = NULL) {
    DrawRoundedRectangle(&roundedRect, brush, strokeWidth, strokeStyle);
  }

  void FillRoundedRectangle(CONST D2D1_ROUNDED_RECT & roundedRect,
                            __in ID2D1Brush * brush) {
    FillRoundedRectangle(&roundedRect, brush);
  }

  void DrawEllipse(CONST D2D1_ELLIPSE & ellipse, __in ID2D1Brush * brush,
                   FLOAT strokeWidth = 1.0f,
                   __in_opt ID2D1StrokeStyle *strokeStyle = NULL) {
    DrawEllipse(&ellipse, brush, strokeWidth, strokeStyle);
  }

  void FillEllipse(CONST D2D1_ELLIPSE & ellipse, __in ID2D1Brush * brush) {
    FillEllipse(&ellipse, brush);
  }

  void FillOpacityMask(__in ID2D1Bitmap * opacityMask, __in ID2D1Brush * brush,
                       D2D1_OPACITY_MASK_CONTENT content,
                       CONST D2D1_RECT_F & destinationRectangle,
                       CONST D2D1_RECT_F & sourceRectangle) {
    FillOpacityMask(opacityMask, brush, content, &destinationRectangle,
                    &sourceRectangle);
  }

  void DrawBitmap(__in ID2D1Bitmap * bitmap,
                  CONST D2D1_RECT_F & destinationRectangle,
                  FLOAT opacity = 1.0f,
                  D2D1_BITMAP_INTERPOLATION_MODE interpolationMode =
                      D2D1_BITMAP_INTERPOLATION_MODE_LINEAR,
                  __in_opt CONST D2D1_RECT_F *sourceRectangle = NULL) {
    DrawBitmap(bitmap, &destinationRectangle, opacity, interpolationMode,
               sourceRectangle);
  }
  ...
  void PushAxisAlignedClip(CONST D2D1_RECT_F & clipRect,
                           D2D1_ANTIALIAS_MODE antialiasMode) {
  ...
  void DrawText(
      __in_ecount(stringLength) CONST WCHAR * string, UINT stringLength,
      __in IDWriteTextFormat * textFormat, CONST D2D1_RECT_F & layoutRect,
      __in ID2D1Brush * defaultForegroundBrush,
      D2D1_DRAW_TEXT_OPTIONS options = D2D1_DRAW_TEXT_OPTIONS_NONE,
      DWRITE_MEASURING_MODE measuringMode = DWRITE_MEASURING_MODE_NATURAL) {
    return DrawText(string, stringLength, textFormat, &layoutRect,
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd90695-12e2-11dc-9fed-001143a055f9")
    ID2D1BitmapRenderTarget : public ID2D1RenderTarget {

  STDMETHOD(GetBitmap)(__deref_out ID2D1Bitmap * *bitmap) PURE;
}; // interface ID2D1BitmapRenderTarget
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("2cd90698-12e2-11dc-9fed-001143a055f9")
  ...
  // if there is insufficent video memory or system memory, or if the render
  // target is resized beyond the maximum bitmap size. If the method fails, the
  // render target will be placed in a zombie state and D2DERR_RECREATE_TARGET
  // will be returned from it when EndDraw is called. In addition an appropriate
  // failure result will be returned from Resize.
  //
  STDMETHOD(Resize)(__in CONST D2D1_SIZE_U * pixelSize) PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("e0db51c3-6f77-4bae-b3d5-e47509b35838")
    ID2D1GdiInteropRenderTarget : public IUnknown {

  STDMETHOD(GetDC)(D2D1_DC_INITIALIZE_MODE mode, __out HDC * hdc) PURE;

  STDMETHOD(ReleaseDC)(__in_opt CONST RECT * update) PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("1c51bc64-de61-46fd-9899-63a5d8f03950")
    ID2D1DCRenderTarget : public ID2D1RenderTarget {

  STDMETHOD(BindDC)(__in CONST HDC hDC, __in CONST RECT * pSubRect) PURE;
...
//
//------------------------------------------------------------------------------
interface D2D1_DECLARE_INTERFACE("06152247-6f50-465a-9245-118bfd3b6007")
    ID2D1Factory : public IUnknown {

  //
  // Cause the factory to refresh any system metrics that it might have been
  // snapped on factory creation.
  //
  STDMETHOD(ReloadSystemMetrics)() PURE;

  //
  // Retrieves the current desktop DPI. To refresh this, call
  // ReloadSystemMetrics.
  //
  STDMETHOD_(void, GetDesktopDpi)(__out FLOAT * dpiX, __out FLOAT * dpiY) PURE;

  STDMETHOD(CreateRectangleGeometry)
  (__in CONST D2D1_RECT_F * rectangle,
   __deref_out ID2D1RectangleGeometry * *rectangleGeometry) PURE;

  STDMETHOD(CreateRoundedRectangleGeometry)
  (__in CONST D2D1_ROUNDED_RECT * roundedRectangle,
   __deref_out ID2D1RoundedRectangleGeometry * *roundedRectangleGeometry) PURE;

  STDMETHOD(CreateEllipseGeometry)
  (__in CONST D2D1_ELLIPSE * ellipse,
   __deref_out ID2D1EllipseGeometry * *ellipseGeometry) PURE;
  ...
  (D2D1_FILL_MODE fillMode,
   ...
   UINT geometriesCount, __deref_out ID2D1GeometryGroup * *geometryGroup) PURE;

  STDMETHOD(CreateTransformedGeometry)
  (__in ID2D1Geometry * sourceGeometry,
   __in CONST D2D1_MATRIX_3X2_F * transform,
   __deref_out ID2D1TransformedGeometry * *transformedGeometry) PURE;

  //
  // Returns an initially empty path geometry interface. A geometry sink is
  // created off the interface to populate it.
  //
  STDMETHOD(CreatePathGeometry)
  (__deref_out ID2D1PathGeometry * *pathGeometry) PURE;
  ...
  (__in CONST D2D1_STROKE_STYLE_PROPERTIES * strokeStyleProperties,
   __in_ecount_opt(dashesCount) CONST FLOAT * dashes, UINT dashesCount,
   __deref_out ID2D1StrokeStyle * *strokeStyle) PURE;
  ...
  (__in_opt CONST D2D1_DRAWING_STATE_DESCRIPTION * drawingStateDescription,
   ...
   __deref_out ID2D1DrawingStateBlock * *drawingStateBlock) PURE;
  ...
  (__in IWICBitmap * target,
   __in CONST D2D1_RENDER_TARGET_PROPERTIES * renderTargetProperties,
   __deref_out ID2D1RenderTarget * *renderTarget) PURE;
  ...
  (__in CONST D2D1_RENDER_TARGET_PROPERTIES * renderTargetProperties,
   ...
   __deref_out ID2D1HwndRenderTarget * *hwndRenderTarget) PURE;

  //
  // Creates a render target that draws to a DXGI Surface. The device that owns
  // the surface is used for rendering.
  //
  ...
  (__in CONST D2D1_RENDER_TARGET_PROPERTIES * renderTargetProperties,
   __deref_out ID2D1DCRenderTarget * *dcRenderTarget) PURE;

  HRESULT
  CreateRectangleGeometry(CONST D2D1_RECT_F & rectangle,
                          __deref_out ID2D1RectangleGeometry *
                              *rectangleGeometry) {
    return CreateRectangleGeometry(&rectangle, rectangleGeometry);
  }

  HRESULT
  CreateRoundedRectangleGeometry(CONST D2D1_ROUNDED_RECT & roundedRectangle,
                                 __deref_out ID2D1RoundedRectangleGeometry *
  ...
  HRESULT
  CreateEllipseGeometry(CONST D2D1_ELLIPSE & ellipse,
  ...
  CreateTransformedGeometry(
      __in ID2D1Geometry * sourceGeometry, CONST D2D1_MATRIX_3X2_F & transform,
  ...
  HRESULT
  CreateStrokeStyle(CONST D2D1_STROKE_STYLE_PROPERTIES & strokeStyleProperties,
  ...
  HRESULT
  CreateDrawingStateBlock(__deref_out ID2D1DrawingStateBlock *
                          *drawingStateBlock) {
    return CreateDrawingStateBlock(NULL, NULL, drawingStateBlock);
  }

  HRESULT
  CreateWicBitmapRenderTarget(__in IWICBitmap * target,
  ...
  CreateHwndRenderTarget(
    ...
    return CreateHwndRenderTarget(
        &renderTargetProperties, &hwndRenderTargetProperties, hwndRenderTarget);
  }

  HRESULT
  CreateDxgiSurfaceRenderTarget(__in IDXGISurface * dxgiSurface,
                                CONST D2D1_RENDER_TARGET_PROPERTIES &
                                    renderTargetProperties,
                                __deref_out ID2D1RenderTarget * *renderTarget) {
...
typedef struct ID2D1ResourceVtbl {
  ...
  (ID2D1Resource *This, __deref_out ID2D1Factory **factory) PURE;
} ID2D1ResourceVtbl;
...
typedef struct ID2D1BitmapVtbl {
  ...
  (ID2D1Bitmap *This, __out FLOAT *dpiX, __out FLOAT *dpiY) PURE;

  STDMETHOD(CopyFromBitmap)
  (ID2D1Bitmap *This, __in_opt CONST D2D1_POINT_2U *destPoint,
   __in ID2D1Bitmap *bitmap, __in_opt CONST D2D1_RECT_U *srcRect) PURE;
  ...
  STDMETHOD(CopyFromMemory)
  (ID2D1Bitmap *This, __in_opt CONST D2D1_RECT_U *dstRect,
   __in CONST void *srcData, UINT32 pitch) PURE;
} ID2D1BitmapVtbl;
...
#define ID2D1Bitmap_CopyFromRenderTarget(This, destPoint, renderTarget,        \
                                         srcRect)                              \
...
typedef struct ID2D1GradientStopCollectionVtbl {
  ...
  (ID2D1GradientStopCollection *This) PURE;

  STDMETHOD_(void, GetGradientStops)
  (ID2D1GradientStopCollection *This,
   __out_ecount(gradientStopsCount) D2D1_GRADIENT_STOP *gradientStops,
   UINT gradientStopsCount) PURE;
...
#define ID2D1GradientStopCollection_GetGradientStops(This, gradientStops,      \
                                                     gradientStopsCount)       \
...
typedef struct ID2D1BrushVtbl {
  ...
  (ID2D1Brush *This, __in CONST D2D1_MATRIX_3X2_F *transform) PURE;

  STDMETHOD_(FLOAT, GetOpacity)(ID2D1Brush *This) PURE;

  STDMETHOD_(void, GetTransform)
  (ID2D1Brush *This, __out D2D1_MATRIX_3X2_F *transform) PURE;
} ID2D1BrushVtbl;
...
typedef struct ID2D1BitmapBrushVtbl {
  ...
  (ID2D1BitmapBrush *This, D2D1_EXTEND_MODE extendModeX) PURE;

  STDMETHOD_(void, SetExtendModeY)
  (ID2D1BitmapBrush *This, D2D1_EXTEND_MODE extendModeY) PURE;
  ...
  (ID2D1BitmapBrush *This, __in ID2D1Bitmap *bitmap) PURE;
  ...
  (ID2D1BitmapBrush *This, __deref_out ID2D1Bitmap **bitmap) PURE;
...
typedef struct ID2D1SolidColorBrushVtbl {
  ...
  (ID2D1SolidColorBrush *This, __in CONST D2D1_COLOR_F *color) PURE;
...
typedef struct ID2D1LinearGradientBrushVtbl {
  ...
  (ID2D1LinearGradientBrush *This, D2D1_POINT_2F startPoint) PURE;

  STDMETHOD_(void, SetEndPoint)
  (ID2D1LinearGradientBrush *This, D2D1_POINT_2F endPoint) PURE;
...
#define ID2D1LinearGradientBrush_GetGradientStopCollection(                    \
...
typedef struct ID2D1RadialGradientBrushVtbl {
  ...
  (ID2D1RadialGradientBrush *This, D2D1_POINT_2F center) PURE;

  STDMETHOD_(void, SetGradientOriginOffset)
  (ID2D1RadialGradientBrush *This, D2D1_POINT_2F gradientOriginOffset) PURE;

  STDMETHOD_(void, SetRadiusX)
  (ID2D1RadialGradientBrush *This, FLOAT radiusX) PURE;

  STDMETHOD_(void, SetRadiusY)
  (ID2D1RadialGradientBrush *This, FLOAT radiusY) PURE;

  STDMETHOD_(D2D1_POINT_2F, GetCenter)(ID2D1RadialGradientBrush *This) PURE;

  STDMETHOD_(D2D1_POINT_2F, GetGradientOriginOffset)
  (ID2D1RadialGradientBrush *This) PURE;
...
#define ID2D1RadialGradientBrush_SetGradientOriginOffset(This,                 \
...
#define ID2D1RadialGradientBrush_GetGradientStopCollection(                    \
    This, gradientStopCollection)                                              \
  ((This)->lpVtbl->GetGradientStopCollection(This, gradientStopCollection))
...
typedef struct ID2D1StrokeStyleVtbl {
  ...
  STDMETHOD_(void, GetDashes)
  (ID2D1StrokeStyle *This, __out_ecount(dashesCount) FLOAT *dashes,
   UINT dashesCount) PURE;
...
typedef struct ID2D1GeometryVtbl {
  ...
  STDMETHOD(GetBounds)
  (ID2D1Geometry *This, __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform,
   __out D2D1_RECT_F *bounds) PURE;
  ...
  (ID2D1Geometry *This, D2D1_POINT_2F point, FLOAT strokeWidth,
   __in_opt ID2D1StrokeStyle *strokeStyle,
   __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform, FLOAT flatteningTolerance,
   __out BOOL *contains) PURE;

  STDMETHOD(FillContainsPoint)
  (ID2D1Geometry *This, D2D1_POINT_2F point,
   __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform, FLOAT flatteningTolerance,
   __out BOOL *contains) PURE;

  STDMETHOD(CompareWithGeometry)
  (ID2D1Geometry *This, __in ID2D1Geometry *inputGeometry,
   __in_opt CONST D2D1_MATRIX_3X2_F *inputGeometryTransform,
   FLOAT flatteningTolerance, __out D2D1_GEOMETRY_RELATION *relation) PURE;

  STDMETHOD(Simplify)
  (ID2D1Geometry *This,
   D2D1_GEOMETRY_SIMPLIFICATION_OPTION simplificationOption,
   __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform, FLOAT flatteningTolerance,
   __in ID2D1SimplifiedGeometrySink *geometrySink) PURE;

  STDMETHOD(Tessellate)
  (ID2D1Geometry *This, __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform,
   FLOAT flatteningTolerance,
   __in ID2D1TessellationSink *tessellationSink) PURE;
  ...
  STDMETHOD(ComputeArea)
  (ID2D1Geometry *This, __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform,
   FLOAT flatteningTolerance, __out FLOAT *area) PURE;

  STDMETHOD(ComputeLength)
  (ID2D1Geometry *This, __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform,
   FLOAT flatteningTolerance, __out FLOAT *length) PURE;

  STDMETHOD(ComputePointAtLength)
  (ID2D1Geometry *This, FLOAT length,
   __in_opt CONST D2D1_MATRIX_3X2_F *worldTransform, FLOAT flatteningTolerance,
   __out_opt D2D1_POINT_2F *point,
   __out_opt D2D1_POINT_2F *unitTangentVector) PURE;
...
#define ID2D1Geometry_GetWidenedBounds(This, strokeWidth, strokeStyle,         \
                                       worldTransform, flatteningTolerance,    \
                                       bounds)                                 \
  ((This)->lpVtbl->GetWidenedBounds(This, strokeWidth, strokeStyle,            \
                                    worldTransform, flatteningTolerance,       \
                                    bounds))

#define ID2D1Geometry_StrokeContainsPoint(This, point, strokeWidth,            \
                                          strokeStyle, worldTransform,         \
                                          flatteningTolerance, contains)       \
  ((This)->lpVtbl->StrokeContainsPoint(This, point, strokeWidth, strokeStyle,  \
                                       worldTransform, flatteningTolerance,    \
                                       contains))

#define ID2D1Geometry_FillContainsPoint(This, point, worldTransform,           \
                                        flatteningTolerance, contains)         \
  ((This)->lpVtbl->FillContainsPoint(This, point, worldTransform,              \
                                     flatteningTolerance, contains))

#define ID2D1Geometry_CompareWithGeometry(This, inputGeometry,                 \
                                          inputGeometryTransform,              \
                                          flatteningTolerance, relation)       \
  ((This)->lpVtbl->CompareWithGeometry(This, inputGeometry,                    \
                                       inputGeometryTransform,                 \
                                       flatteningTolerance, relation))

#define ID2D1Geometry_Simplify(This, simplificationOption, worldTransform,     \
                               flatteningTolerance, geometrySink)              \
  ((This)->lpVtbl->Simplify(This, simplificationOption, worldTransform,        \
                            flatteningTolerance, geometrySink))

#define ID2D1Geometry_Tessellate(This, worldTransform, flatteningTolerance,    \
                                 tessellationSink)                             \
  ((This)->lpVtbl->Tessellate(This, worldTransform, flatteningTolerance,       \
                              tessellationSink))

#define ID2D1Geometry_CombineWithGeometry(This, inputGeometry, combineMode,    \
                                          inputGeometryTransform,              \
                                          flatteningTolerance, geometrySink)   \
  ((This)->lpVtbl->CombineWithGeometry(This, inputGeometry, combineMode,       \
                                       inputGeometryTransform,                 \
                                       flatteningTolerance, geometrySink))

#define ID2D1Geometry_Outline(This, worldTransform, flatteningTolerance,       \
                              geometrySink)                                    \
  ((This)->lpVtbl->Outline(This, worldTransform, flatteningTolerance,          \
                           geometrySink))

#define ID2D1Geometry_ComputeArea(This, worldTransform, flatteningTolerance,   \
                                  area)                                        \
  ((This)->lpVtbl->ComputeArea(This, worldTransform, flatteningTolerance, area))

#define ID2D1Geometry_ComputeLength(This, worldTransform, flatteningTolerance, \
                                    length)                                    \
  ((This)->lpVtbl->ComputeLength(This, worldTransform, flatteningTolerance,    \
                                 length))

#define ID2D1Geometry_ComputePointAtLength(This, length, worldTransform,       \
                                           flatteningTolerance, point,         \
                                           unitTangentVector)                  \
  ((This)->lpVtbl->ComputePointAtLength(This, length, worldTransform,          \
                                        flatteningTolerance, point,            \
                                        unitTangentVector))

#define ID2D1Geometry_Widen(This, strokeWidth, strokeStyle, worldTransform,    \
                            flatteningTolerance, geometrySink)                 \
  ((This)->lpVtbl->Widen(This, strokeWidth, strokeStyle, worldTransform,       \
                         flatteningTolerance, geometrySink))
...
typedef struct ID2D1RectangleGeometryVtbl {
  ...
  (ID2D1RectangleGeometry *This, __out D2D1_RECT_F *rect) PURE;
...
#define ID2D1RectangleGeometry_GetBounds(This, worldTransform, bounds)         \
  ((This)->lpVtbl->Base.GetBounds((ID2D1Geometry *)This, worldTransform,       \
                                  bounds))

#define ID2D1RectangleGeometry_GetWidenedBounds(This, strokeWidth,             \
                                                strokeStyle, worldTransform,   \
                                                flatteningTolerance, bounds)   \
  ((This)->lpVtbl->Base.GetWidenedBounds((ID2D1Geometry *)This, strokeWidth,   \
                                         strokeStyle, worldTransform,          \
                                         flatteningTolerance, bounds))

#define ID2D1RectangleGeometry_StrokeContainsPoint(                            \
    This, point, strokeWidth, strokeStyle, worldTransform,                     \
    flatteningTolerance, contains)                                             \
  ((This)->lpVtbl->Base.StrokeContainsPoint(                                   \
      (ID2D1Geometry *)This, point, strokeWidth, strokeStyle, worldTransform,  \
      flatteningTolerance, contains))

#define ID2D1RectangleGeometry_FillContainsPoint(                              \
    This, point, worldTransform, flatteningTolerance, contains)                \
  ((This)->lpVtbl->Base.FillContainsPoint((ID2D1Geometry *)This, point,        \
                                          worldTransform, flatteningTolerance, \
                                          contains))

#define ID2D1RectangleGeometry_CompareWithGeometry(                            \
    This, inputGeometry, inputGeometryTransform, flatteningTolerance,          \
    relation)                                                                  \
  ((This)->lpVtbl->Base.CompareWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, inputGeometryTransform,            \
      flatteningTolerance, relation))

#define ID2D1RectangleGeometry_Simplify(This, simplificationOption,            \
                                        worldTransform, flatteningTolerance,   \
                                        geometrySink)                          \
...
#define ID2D1RectangleGeometry_Tessellate(                                     \
    This, worldTransform, flatteningTolerance, tessellationSink)               \
  ((This)->lpVtbl->Base.Tessellate((ID2D1Geometry *)This, worldTransform,      \
                                   flatteningTolerance, tessellationSink))

#define ID2D1RectangleGeometry_CombineWithGeometry(                            \
    This, inputGeometry, combineMode, inputGeometryTransform,                  \
    flatteningTolerance, geometrySink)                                         \
  ((This)->lpVtbl->Base.CombineWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, combineMode,                       \
      inputGeometryTransform, flatteningTolerance, geometrySink))

#define ID2D1RectangleGeometry_Outline(This, worldTransform,                   \
                                       flatteningTolerance, geometrySink)      \
  ((This)->lpVtbl->Base.Outline((ID2D1Geometry *)This, worldTransform,         \
                                flatteningTolerance, geometrySink))

#define ID2D1RectangleGeometry_ComputeArea(This, worldTransform,               \
                                           flatteningTolerance, area)          \
  ((This)->lpVtbl->Base.ComputeArea((ID2D1Geometry *)This, worldTransform,     \
                                    flatteningTolerance, area))

#define ID2D1RectangleGeometry_ComputeLength(This, worldTransform,             \
                                             flatteningTolerance, length)      \
  ((This)->lpVtbl->Base.ComputeLength((ID2D1Geometry *)This, worldTransform,   \
                                      flatteningTolerance, length))

#define ID2D1RectangleGeometry_ComputePointAtLength(                           \
    This, length, worldTransform, flatteningTolerance, point,                  \
    unitTangentVector)                                                         \
  ((This)->lpVtbl->Base.ComputePointAtLength(                                  \
      (ID2D1Geometry *)This, length, worldTransform, flatteningTolerance,      \
      point, unitTangentVector))

#define ID2D1RectangleGeometry_Widen(This, strokeWidth, strokeStyle,           \
                                     worldTransform, flatteningTolerance,      \
                                     geometrySink)                             \
...
typedef struct ID2D1RoundedRectangleGeometryVtbl {
  ...
  (ID2D1RoundedRectangleGeometry *This,
   __out D2D1_ROUNDED_RECT *roundedRect) PURE;
...
#define ID2D1RoundedRectangleGeometry_GetWidenedBounds(                        \
    This, strokeWidth, strokeStyle, worldTransform, flatteningTolerance,       \
    bounds)                                                                    \
  ((This)->lpVtbl->Base.GetWidenedBounds((ID2D1Geometry *)This, strokeWidth,   \
                                         strokeStyle, worldTransform,          \
                                         flatteningTolerance, bounds))

#define ID2D1RoundedRectangleGeometry_StrokeContainsPoint(                     \
    This, point, strokeWidth, strokeStyle, worldTransform,                     \
    flatteningTolerance, contains)                                             \
  ((This)->lpVtbl->Base.StrokeContainsPoint(                                   \
      (ID2D1Geometry *)This, point, strokeWidth, strokeStyle, worldTransform,  \
...
#define ID2D1RoundedRectangleGeometry_Simplify(                                \
    This, simplificationOption, worldTransform, flatteningTolerance,           \
    geometrySink)                                                              \
  ((This)->lpVtbl->Base.Simplify((ID2D1Geometry *)This, simplificationOption,  \
                                 worldTransform, flatteningTolerance,          \
                                 geometrySink))
...
#define ID2D1RoundedRectangleGeometry_CombineWithGeometry(                     \
    This, inputGeometry, combineMode, inputGeometryTransform,                  \
    flatteningTolerance, geometrySink)                                         \
  ((This)->lpVtbl->Base.CombineWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, combineMode,                       \
...
#define ID2D1RoundedRectangleGeometry_Outline(                                 \
    This, worldTransform, flatteningTolerance, geometrySink)                   \
  ((This)->lpVtbl->Base.Outline((ID2D1Geometry *)This, worldTransform,         \
                                flatteningTolerance, geometrySink))

#define ID2D1RoundedRectangleGeometry_ComputeArea(This, worldTransform,        \
                                                  flatteningTolerance, area)   \
  ((This)->lpVtbl->Base.ComputeArea((ID2D1Geometry *)This, worldTransform,     \
                                    flatteningTolerance, area))

#define ID2D1RoundedRectangleGeometry_ComputeLength(                           \
    This, worldTransform, flatteningTolerance, length)                         \
  ((This)->lpVtbl->Base.ComputeLength((ID2D1Geometry *)This, worldTransform,   \
                                      flatteningTolerance, length))

#define ID2D1RoundedRectangleGeometry_ComputePointAtLength(                    \
    This, length, worldTransform, flatteningTolerance, point,                  \
    unitTangentVector)                                                         \
  ((This)->lpVtbl->Base.ComputePointAtLength(                                  \
      (ID2D1Geometry *)This, length, worldTransform, flatteningTolerance,      \
...
#define ID2D1RoundedRectangleGeometry_Widen(This, strokeWidth, strokeStyle,    \
                                            worldTransform,                    \
                                            flatteningTolerance, geometrySink) \
  ((This)->lpVtbl->Base.Widen((ID2D1Geometry *)This, strokeWidth, strokeStyle, \
                              worldTransform, flatteningTolerance,             \
...
#define ID2D1EllipseGeometry_GetWidenedBounds(This, strokeWidth, strokeStyle,  \
                                              worldTransform,                  \
                                              flatteningTolerance, bounds)     \
  ((This)->lpVtbl->Base.GetWidenedBounds((ID2D1Geometry *)This, strokeWidth,   \
                                         strokeStyle, worldTransform,          \
...
#define ID2D1EllipseGeometry_StrokeContainsPoint(                              \
    This, point, strokeWidth, strokeStyle, worldTransform,                     \
    flatteningTolerance, contains)                                             \
  ((This)->lpVtbl->Base.StrokeContainsPoint(                                   \
      (ID2D1Geometry *)This, point, strokeWidth, strokeStyle, worldTransform,  \
...
#define ID2D1EllipseGeometry_FillContainsPoint(This, point, worldTransform,    \
                                               flatteningTolerance, contains)  \
  ((This)->lpVtbl->Base.FillContainsPoint((ID2D1Geometry *)This, point,        \
                                          worldTransform, flatteningTolerance, \
                                          contains))

#define ID2D1EllipseGeometry_CompareWithGeometry(                              \
    This, inputGeometry, inputGeometryTransform, flatteningTolerance,          \
    relation)                                                                  \
  ((This)->lpVtbl->Base.CompareWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, inputGeometryTransform,            \
...
#define ID2D1EllipseGeometry_Simplify(This, simplificationOption,              \
                                      worldTransform, flatteningTolerance,     \
                                      geometrySink)                            \
  ((This)->lpVtbl->Base.Simplify((ID2D1Geometry *)This, simplificationOption,  \
                                 worldTransform, flatteningTolerance,          \
...
#define ID2D1EllipseGeometry_Tessellate(This, worldTransform,                  \
                                        flatteningTolerance, tessellationSink) \
  ((This)->lpVtbl->Base.Tessellate((ID2D1Geometry *)This, worldTransform,      \
                                   flatteningTolerance, tessellationSink))

#define ID2D1EllipseGeometry_CombineWithGeometry(                              \
    This, inputGeometry, combineMode, inputGeometryTransform,                  \
    flatteningTolerance, geometrySink)                                         \
  ((This)->lpVtbl->Base.CombineWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, combineMode,                       \
...
#define ID2D1EllipseGeometry_Outline(This, worldTransform,                     \
                                     flatteningTolerance, geometrySink)        \
  ((This)->lpVtbl->Base.Outline((ID2D1Geometry *)This, worldTransform,         \
                                flatteningTolerance, geometrySink))

#define ID2D1EllipseGeometry_ComputeArea(This, worldTransform,                 \
                                         flatteningTolerance, area)            \
  ((This)->lpVtbl->Base.ComputeArea((ID2D1Geometry *)This, worldTransform,     \
                                    flatteningTolerance, area))

#define ID2D1EllipseGeometry_ComputeLength(This, worldTransform,               \
                                           flatteningTolerance, length)        \
  ((This)->lpVtbl->Base.ComputeLength((ID2D1Geometry *)This, worldTransform,   \
                                      flatteningTolerance, length))

#define ID2D1EllipseGeometry_ComputePointAtLength(                             \
    This, length, worldTransform, flatteningTolerance, point,                  \
    unitTangentVector)                                                         \
  ((This)->lpVtbl->Base.ComputePointAtLength(                                  \
      (ID2D1Geometry *)This, length, worldTransform, flatteningTolerance,      \
...
#define ID2D1EllipseGeometry_Widen(This, strokeWidth, strokeStyle,             \
                                   worldTransform, flatteningTolerance,        \
                                   geometrySink)                               \
  ((This)->lpVtbl->Base.Widen((ID2D1Geometry *)This, strokeWidth, strokeStyle, \
                              worldTransform, flatteningTolerance,             \
...
#define ID2D1GeometryGroup_GetWidenedBounds(This, strokeWidth, strokeStyle,    \
                                            worldTransform,                    \
                                            flatteningTolerance, bounds)       \
  ((This)->lpVtbl->Base.GetWidenedBounds((ID2D1Geometry *)This, strokeWidth,   \
                                         strokeStyle, worldTransform,          \
...
#define ID2D1GeometryGroup_StrokeContainsPoint(This, point, strokeWidth,       \
                                               strokeStyle, worldTransform,    \
                                               flatteningTolerance, contains)  \
  ((This)->lpVtbl->Base.StrokeContainsPoint(                                   \
      (ID2D1Geometry *)This, point, strokeWidth, strokeStyle, worldTransform,  \
...
#define ID2D1GeometryGroup_FillContainsPoint(This, point, worldTransform,      \
                                             flatteningTolerance, contains)    \
  ((This)->lpVtbl->Base.FillContainsPoint((ID2D1Geometry *)This, point,        \
                                          worldTransform, flatteningTolerance, \
                                          contains))

#define ID2D1GeometryGroup_CompareWithGeometry(This, inputGeometry,            \
                                               inputGeometryTransform,         \
                                               flatteningTolerance, relation)  \
  ((This)->lpVtbl->Base.CompareWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, inputGeometryTransform,            \
...
#define ID2D1GeometryGroup_Simplify(This, simplificationOption,                \
                                    worldTransform, flatteningTolerance,       \
                                    geometrySink)                              \
  ((This)->lpVtbl->Base.Simplify((ID2D1Geometry *)This, simplificationOption,  \
                                 worldTransform, flatteningTolerance,          \
...
#define ID2D1GeometryGroup_Tessellate(This, worldTransform,                    \
                                      flatteningTolerance, tessellationSink)   \
  ((This)->lpVtbl->Base.Tessellate((ID2D1Geometry *)This, worldTransform,      \
                                   flatteningTolerance, tessellationSink))

#define ID2D1GeometryGroup_CombineWithGeometry(                                \
    This, inputGeometry, combineMode, inputGeometryTransform,                  \
    flatteningTolerance, geometrySink)                                         \
  ((This)->lpVtbl->Base.CombineWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, combineMode,                       \
...
#define ID2D1GeometryGroup_Outline(This, worldTransform, flatteningTolerance,  \
                                   geometrySink)                               \
  ((This)->lpVtbl->Base.Outline((ID2D1Geometry *)This, worldTransform,         \
                                flatteningTolerance, geometrySink))

#define ID2D1GeometryGroup_ComputeArea(This, worldTransform,                   \
                                       flatteningTolerance, area)              \
  ((This)->lpVtbl->Base.ComputeArea((ID2D1Geometry *)This, worldTransform,     \
                                    flatteningTolerance, area))

#define ID2D1GeometryGroup_ComputeLength(This, worldTransform,                 \
                                         flatteningTolerance, length)          \
  ((This)->lpVtbl->Base.ComputeLength((ID2D1Geometry *)This, worldTransform,   \
                                      flatteningTolerance, length))

#define ID2D1GeometryGroup_ComputePointAtLength(This, length, worldTransform,  \
                                                flatteningTolerance, point,    \
                                                unitTangentVector)             \
  ((This)->lpVtbl->Base.ComputePointAtLength(                                  \
      (ID2D1Geometry *)This, length, worldTransform, flatteningTolerance,      \
...
#define ID2D1GeometryGroup_Widen(This, strokeWidth, strokeStyle,               \
                                 worldTransform, flatteningTolerance,          \
                                 geometrySink)                                 \
  ((This)->lpVtbl->Base.Widen((ID2D1Geometry *)This, strokeWidth, strokeStyle, \
                              worldTransform, flatteningTolerance,             \
...
#define ID2D1GeometryGroup_GetSourceGeometries(This, geometries,               \
                                               geometriesCount)                \
...
typedef struct ID2D1TransformedGeometryVtbl {
  ...
  (ID2D1TransformedGeometry *This,
   __deref_out ID2D1Geometry **sourceGeometry) PURE;

  STDMETHOD_(void, GetTransform)
  (ID2D1TransformedGeometry *This, __out D2D1_MATRIX_3X2_F *transform) PURE;
...
#define ID2D1TransformedGeometry_GetWidenedBounds(This, strokeWidth,           \
                                                  strokeStyle, worldTransform, \
                                                  flatteningTolerance, bounds) \
  ((This)->lpVtbl->Base.GetWidenedBounds((ID2D1Geometry *)This, strokeWidth,   \
                                         strokeStyle, worldTransform,          \
...
#define ID2D1TransformedGeometry_StrokeContainsPoint(                          \
    This, point, strokeWidth, strokeStyle, worldTransform,                     \
    flatteningTolerance, contains)                                             \
  ((This)->lpVtbl->Base.StrokeContainsPoint(                                   \
      (ID2D1Geometry *)This, point, strokeWidth, strokeStyle, worldTransform,  \
...
#define ID2D1TransformedGeometry_FillContainsPoint(                            \
    This, point, worldTransform, flatteningTolerance, contains)                \
  ((This)->lpVtbl->Base.FillContainsPoint((ID2D1Geometry *)This, point,        \
                                          worldTransform, flatteningTolerance, \
                                          contains))

#define ID2D1TransformedGeometry_CompareWithGeometry(                          \
    This, inputGeometry, inputGeometryTransform, flatteningTolerance,          \
    relation)                                                                  \
  ((This)->lpVtbl->Base.CompareWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, inputGeometryTransform,            \
...
#define ID2D1TransformedGeometry_Simplify(This, simplificationOption,          \
                                          worldTransform, flatteningTolerance, \
                                          geometrySink)                        \
  ((This)->lpVtbl->Base.Simplify((ID2D1Geometry *)This, simplificationOption,  \
                                 worldTransform, flatteningTolerance,          \
...
#define ID2D1TransformedGeometry_Tessellate(                                   \
    This, worldTransform, flatteningTolerance, tessellationSink)               \
  ((This)->lpVtbl->Base.Tessellate((ID2D1Geometry *)This, worldTransform,      \
                                   flatteningTolerance, tessellationSink))
...
#define ID2D1TransformedGeometry_Outline(This, worldTransform,                 \
                                         flatteningTolerance, geometrySink)    \
  ((This)->lpVtbl->Base.Outline((ID2D1Geometry *)This, worldTransform,         \
                                flatteningTolerance, geometrySink))

#define ID2D1TransformedGeometry_ComputeArea(This, worldTransform,             \
                                             flatteningTolerance, area)        \
  ((This)->lpVtbl->Base.ComputeArea((ID2D1Geometry *)This, worldTransform,     \
                                    flatteningTolerance, area))

#define ID2D1TransformedGeometry_ComputeLength(This, worldTransform,           \
                                               flatteningTolerance, length)    \
  ((This)->lpVtbl->Base.ComputeLength((ID2D1Geometry *)This, worldTransform,   \
                                      flatteningTolerance, length))

#define ID2D1TransformedGeometry_ComputePointAtLength(                         \
    This, length, worldTransform, flatteningTolerance, point,                  \
    unitTangentVector)                                                         \
  ((This)->lpVtbl->Base.ComputePointAtLength(                                  \
      (ID2D1Geometry *)This, length, worldTransform, flatteningTolerance,      \
...
#define ID2D1TransformedGeometry_Widen(This, strokeWidth, strokeStyle,         \
                                       worldTransform, flatteningTolerance,    \
                                       geometrySink)                           \
  ((This)->lpVtbl->Base.Widen((ID2D1Geometry *)This, strokeWidth, strokeStyle, \
                              worldTransform, flatteningTolerance,             \
...
typedef struct ID2D1SimplifiedGeometrySinkVtbl {
  ...
  (ID2D1SimplifiedGeometrySink *This, D2D1_FILL_MODE fillMode) PURE;

  STDMETHOD_(void, SetSegmentFlags)
  (ID2D1SimplifiedGeometrySink *This, D2D1_PATH_SEGMENT vertexFlags) PURE;

  STDMETHOD_(void, BeginFigure)
  (ID2D1SimplifiedGeometrySink *This, D2D1_POINT_2F startPoint,
   D2D1_FIGURE_BEGIN figureBegin) PURE;

  STDMETHOD_(void, AddLines)
  (ID2D1SimplifiedGeometrySink *This,
   __in_ecount(pointsCount) CONST D2D1_POINT_2F *points, UINT pointsCount) PURE;
  ...
  (ID2D1SimplifiedGeometrySink *This, D2D1_FIGURE_END figureEnd) PURE;
...
typedef struct ID2D1GeometrySinkVtbl {
  ...
  (ID2D1GeometrySink *This, __in CONST D2D1_BEZIER_SEGMENT *bezier) PURE;
  ...
  (ID2D1GeometrySink *This, __in CONST D2D1_ARC_SEGMENT *arc) PURE;
...
#define ID2D1GeometrySink_SetFillMode(This, fillMode)                          \
  ((This)->lpVtbl->Base.SetFillMode((ID2D1SimplifiedGeometrySink *)This,       \
                                    fillMode))

#define ID2D1GeometrySink_SetSegmentFlags(This, vertexFlags)                   \
  ((This)->lpVtbl->Base.SetSegmentFlags((ID2D1SimplifiedGeometrySink *)This,   \
                                        vertexFlags))

#define ID2D1GeometrySink_BeginFigure(This, startPoint, figureBegin)           \
  ((This)->lpVtbl->Base.BeginFigure((ID2D1SimplifiedGeometrySink *)This,       \
                                    startPoint, figureBegin))
...
#define ID2D1GeometrySink_AddBeziers(This, beziers, beziersCount)              \
  ((This)->lpVtbl->Base.AddBeziers((ID2D1SimplifiedGeometrySink *)This,        \
                                   beziers, beziersCount))

#define ID2D1GeometrySink_EndFigure(This, figureEnd)                           \
  ((This)->lpVtbl->Base.EndFigure((ID2D1SimplifiedGeometrySink *)This,         \
                                  figureEnd))
...
typedef struct ID2D1PathGeometryVtbl {
  ...
  STDMETHOD(Open)
  (ID2D1PathGeometry *This, __deref_out ID2D1GeometrySink **geometrySink) PURE;

  STDMETHOD(Stream)
  (ID2D1PathGeometry *This, __in ID2D1GeometrySink *geometrySink) PURE;

  STDMETHOD(GetSegmentCount)(ID2D1PathGeometry *This, __out UINT32 *count) PURE;

  STDMETHOD(GetFigureCount)(ID2D1PathGeometry *This, __out UINT32 *count) PURE;
...
#define ID2D1PathGeometry_GetWidenedBounds(This, strokeWidth, strokeStyle,     \
                                           worldTransform,                     \
                                           flatteningTolerance, bounds)        \
  ((This)->lpVtbl->Base.GetWidenedBounds((ID2D1Geometry *)This, strokeWidth,   \
                                         strokeStyle, worldTransform,          \
...
#define ID2D1PathGeometry_StrokeContainsPoint(This, point, strokeWidth,        \
                                              strokeStyle, worldTransform,     \
                                              flatteningTolerance, contains)   \
  ((This)->lpVtbl->Base.StrokeContainsPoint(                                   \
      (ID2D1Geometry *)This, point, strokeWidth, strokeStyle, worldTransform,  \
...
#define ID2D1PathGeometry_FillContainsPoint(This, point, worldTransform,       \
                                            flatteningTolerance, contains)     \
  ((This)->lpVtbl->Base.FillContainsPoint((ID2D1Geometry *)This, point,        \
                                          worldTransform, flatteningTolerance, \
                                          contains))

#define ID2D1PathGeometry_CompareWithGeometry(This, inputGeometry,             \
                                              inputGeometryTransform,          \
                                              flatteningTolerance, relation)   \
  ((This)->lpVtbl->Base.CompareWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, inputGeometryTransform,            \
...
#define ID2D1PathGeometry_Simplify(This, simplificationOption, worldTransform, \
                                   flatteningTolerance, geometrySink)          \
  ((This)->lpVtbl->Base.Simplify((ID2D1Geometry *)This, simplificationOption,  \
                                 worldTransform, flatteningTolerance,          \
                                 geometrySink))

#define ID2D1PathGeometry_Tessellate(This, worldTransform,                     \
                                     flatteningTolerance, tessellationSink)    \
  ((This)->lpVtbl->Base.Tessellate((ID2D1Geometry *)This, worldTransform,      \
                                   flatteningTolerance, tessellationSink))

#define ID2D1PathGeometry_CombineWithGeometry(                                 \
    This, inputGeometry, combineMode, inputGeometryTransform,                  \
    flatteningTolerance, geometrySink)                                         \
  ((This)->lpVtbl->Base.CombineWithGeometry(                                   \
      (ID2D1Geometry *)This, inputGeometry, combineMode,                       \
...
#define ID2D1PathGeometry_Outline(This, worldTransform, flatteningTolerance,   \
                                  geometrySink)                                \
  ((This)->lpVtbl->Base.Outline((ID2D1Geometry *)This, worldTransform,         \
                                flatteningTolerance, geometrySink))

#define ID2D1PathGeometry_ComputeArea(This, worldTransform,                    \
                                      flatteningTolerance, area)               \
  ((This)->lpVtbl->Base.ComputeArea((ID2D1Geometry *)This, worldTransform,     \
                                    flatteningTolerance, area))

#define ID2D1PathGeometry_ComputeLength(This, worldTransform,                  \
                                        flatteningTolerance, length)           \
  ((This)->lpVtbl->Base.ComputeLength((ID2D1Geometry *)This, worldTransform,   \
                                      flatteningTolerance, length))

#define ID2D1PathGeometry_ComputePointAtLength(This, length, worldTransform,   \
                                               flatteningTolerance, point,     \
                                               unitTangentVector)              \
  ((This)->lpVtbl->Base.ComputePointAtLength(                                  \
      (ID2D1Geometry *)This, length, worldTransform, flatteningTolerance,      \
...
#define ID2D1PathGeometry_Widen(This, strokeWidth, strokeStyle,                \
                                worldTransform, flatteningTolerance,           \
                                geometrySink)                                  \
  ((This)->lpVtbl->Base.Widen((ID2D1Geometry *)This, strokeWidth, strokeStyle, \
                              worldTransform, flatteningTolerance,             \
...
typedef struct ID2D1MeshVtbl {
  ...
  STDMETHOD(Open)
  (ID2D1Mesh *This, __deref_out ID2D1TessellationSink **tessellationSink) PURE;
} ID2D1MeshVtbl;
...
typedef struct ID2D1DrawingStateBlockVtbl {
  ...
  (ID2D1DrawingStateBlock *This,
   __out D2D1_DRAWING_STATE_DESCRIPTION *stateDescription) PURE;
  ...
  STDMETHOD_(void, SetTextRenderingParams)
  (ID2D1DrawingStateBlock *This,
   __in_opt IDWriteRenderingParams *textRenderingParams) PURE;

  STDMETHOD_(void, GetTextRenderingParams)
  (ID2D1DrawingStateBlock *This,
   __deref_out_opt IDWriteRenderingParams **textRenderingParams) PURE;
...
#define ID2D1DrawingStateBlock_SetTextRenderingParams(This,                    \
                                                      textRenderingParams)     \
  ((This)->lpVtbl->SetTextRenderingParams(This, textRenderingParams))

#define ID2D1DrawingStateBlock_GetTextRenderingParams(This,                    \
                                                      textRenderingParams)     \
...
typedef struct ID2D1RenderTargetVtbl {
  ...
  STDMETHOD(CreateBitmap)
  (ID2D1RenderTarget *This, D2D1_SIZE_U size, __in_opt CONST void *srcData,
   UINT32 pitch, __in CONST D2D1_BITMAP_PROPERTIES *bitmapProperties,
   __deref_out ID2D1Bitmap **bitmap) PURE;

  STDMETHOD(CreateBitmapFromWicBitmap)
  (ID2D1RenderTarget *This, __in IWICBitmapSource *wicBitmapSource,
   __in_opt CONST D2D1_BITMAP_PROPERTIES *bitmapProperties,
   __deref_out ID2D1Bitmap **bitmap) PURE;

  STDMETHOD(CreateSharedBitmap)
  (ID2D1RenderTarget *This, __in REFIID riid, __inout void *data,
   __in_opt CONST D2D1_BITMAP_PROPERTIES *bitmapProperties,
   __deref_out ID2D1Bitmap **bitmap) PURE;

  STDMETHOD(CreateBitmapBrush)
  (ID2D1RenderTarget *This, __in ID2D1Bitmap *bitmap,
   __in_opt CONST D2D1_BITMAP_BRUSH_PROPERTIES *bitmapBrushProperties,
   __in_opt CONST D2D1_BRUSH_PROPERTIES *brushProperties,
   __deref_out ID2D1BitmapBrush **bitmapBrush) PURE;

  STDMETHOD(CreateSolidColorBrush)
  (ID2D1RenderTarget *This, __in CONST D2D1_COLOR_F *color,
   __in_opt CONST D2D1_BRUSH_PROPERTIES *brushProperties,
   __deref_out ID2D1SolidColorBrush **solidColorBrush) PURE;
  ...
  (ID2D1RenderTarget *This,
   __in_ecount(gradientStopsCount) CONST D2D1_GRADIENT_STOP *gradientStops,
   __range(>=, 1) UINT gradientStopsCount, D2D1_GAMMA colorInterpolationGamma,
   D2D1_EXTEND_MODE extendMode,
   __deref_out ID2D1GradientStopCollection **gradientStopCollection) PURE;
  ...
  STDMETHOD(CreateCompatibleRenderTarget)
  (ID2D1RenderTarget *This, __in_opt CONST D2D1_SIZE_F *desiredSize,
   __in_opt CONST D2D1_SIZE_U *desiredPixelSize,
   __in_opt CONST D2D1_PIXEL_FORMAT *desiredFormat,
   D2D1_COMPATIBLE_RENDER_TARGET_OPTIONS options,
   __deref_out ID2D1BitmapRenderTarget **bitmapRenderTarget) PURE;
  ...
  STDMETHOD(CreateLayer)
  (ID2D1RenderTarget *This, __in_opt CONST D2D1_SIZE_F *size,
   __deref_out ID2D1Layer **layer) PURE;

  STDMETHOD(CreateMesh)
  (ID2D1RenderTarget *This, __deref_out ID2D1Mesh **mesh) PURE;

  STDMETHOD_(void, DrawLine)
  (ID2D1RenderTarget *This, D2D1_POINT_2F point0, D2D1_POINT_2F point1,
   __in ID2D1Brush *brush, FLOAT strokeWidth,
   __in_opt ID2D1StrokeStyle *strokeStyle) PURE;

  STDMETHOD_(void, DrawRectangle)
  (ID2D1RenderTarget *This, __in CONST D2D1_RECT_F *rect,
   __in ID2D1Brush *brush, FLOAT strokeWidth,
   __in_opt ID2D1StrokeStyle *strokeStyle) PURE;

  STDMETHOD_(void, FillRectangle)
  (ID2D1RenderTarget *This, __in CONST D2D1_RECT_F *rect,
   __in ID2D1Brush *brush) PURE;

  STDMETHOD_(void, DrawRoundedRectangle)
  (ID2D1RenderTarget *This, __in CONST D2D1_ROUNDED_RECT *roundedRect,
   __in ID2D1Brush *brush, FLOAT strokeWidth,
   __in_opt ID2D1StrokeStyle *strokeStyle) PURE;
  ...
  STDMETHOD_(void, DrawEllipse)
  (ID2D1RenderTarget *This, __in CONST D2D1_ELLIPSE *ellipse,
   __in ID2D1Brush *brush, FLOAT strokeWidth,
   __in_opt ID2D1StrokeStyle *strokeStyle) PURE;
  ...
  STDMETHOD_(void, DrawGeometry)
  (ID2D1RenderTarget *This, __in ID2D1Geometry *geometry,
   __in ID2D1Brush *brush, FLOAT strokeWidth,
   __in_opt ID2D1StrokeStyle *strokeStyle) PURE;

  STDMETHOD_(void, FillGeometry)
  (ID2D1RenderTarget *This, __in ID2D1Geometry *geometry,
   __in ID2D1Brush *brush, __in_opt ID2D1Brush *opacityBrush) PURE;

  STDMETHOD_(void, FillMesh)
  (ID2D1RenderTarget *This, __in ID2D1Mesh *mesh, __in ID2D1Brush *brush) PURE;

  STDMETHOD_(void, FillOpacityMask)
  (ID2D1RenderTarget *This, __in ID2D1Bitmap *opacityMask,
   __in ID2D1Brush *brush, D2D1_OPACITY_MASK_CONTENT content,
   __in_opt CONST D2D1_RECT_F *destinationRectangle,
   __in_opt CONST D2D1_RECT_F *sourceRectangle) PURE;

  STDMETHOD_(void, DrawBitmap)
  (ID2D1RenderTarget *This, __in ID2D1Bitmap *bitmap,
   __in_opt CONST D2D1_RECT_F *destinationRectangle, FLOAT opacity,
   D2D1_BITMAP_INTERPOLATION_MODE interpolationMode,
   __in_opt CONST D2D1_RECT_F *sourceRectangle) PURE;

  STDMETHOD_(void, DrawText)
  (ID2D1RenderTarget *This, __in_ecount(stringLength) CONST WCHAR *string,
   UINT stringLength, __in IDWriteTextFormat *textFormat,
   __in CONST D2D1_RECT_F *layoutRect, __in ID2D1Brush *defaultForegroundBrush,
   D2D1_DRAW_TEXT_OPTIONS options, DWRITE_MEASURING_MODE measuringMode) PURE;

  STDMETHOD_(void, DrawTextLayout)
  (ID2D1RenderTarget *This, D2D1_POINT_2F origin,
   __in IDWriteTextLayout *textLayout, __in ID2D1Brush *defaultForegroundBrush,
   D2D1_DRAW_TEXT_OPTIONS options) PURE;

  STDMETHOD_(void, DrawGlyphRun)
  (ID2D1RenderTarget *This, D2D1_POINT_2F baselineOrigin,
   __in CONST DWRITE_GLYPH_RUN *glyphRun, __in ID2D1Brush *foregroundBrush,
   DWRITE_MEASURING_MODE measuringMode) PURE;

  STDMETHOD_(void, SetTransform)
  (ID2D1RenderTarget *This, __in CONST D2D1_MATRIX_3X2_F *transform) PURE;

  STDMETHOD_(void, GetTransform)
  (ID2D1RenderTarget *This, __out D2D1_MATRIX_3X2_F *transform) PURE;

  STDMETHOD_(void, SetAntialiasMode)
  (ID2D1RenderTarget *This, D2D1_ANTIALIAS_MODE antialiasMode) PURE;

  STDMETHOD_(D2D1_ANTIALIAS_MODE, GetAntialiasMode)
  (ID2D1RenderTarget *This) PURE;

  STDMETHOD_(void, SetTextAntialiasMode)
  (ID2D1RenderTarget *This, D2D1_TEXT_ANTIALIAS_MODE textAntialiasMode) PURE;
  ...
  (ID2D1RenderTarget *This, D2D1_TAG tag1, D2D1_TAG tag2) PURE;

  STDMETHOD_(void, GetTags)
  (ID2D1RenderTarget *This, __out_opt D2D1_TAG *tag1,
   __out_opt D2D1_TAG *tag2) PURE;

  STDMETHOD_(void, PushLayer)
  (ID2D1RenderTarget *This, __in CONST D2D1_LAYER_PARAMETERS *layerParameters,
   __in ID2D1Layer *layer) PURE;
  ...
  STDMETHOD_(void, SaveDrawingState)
  (ID2D1RenderTarget *This,
   __inout ID2D1DrawingStateBlock *drawingStateBlock) PURE;
  ...
  STDMETHOD_(void, PushAxisAlignedClip)
  (ID2D1RenderTarget *This, __in CONST D2D1_RECT_F *clipRect,
   D2D1_ANTIALIAS_MODE antialiasMode) PURE;

  STDMETHOD_(void, PopAxisAlignedClip)(ID2D1RenderTarget *This) PURE;

  STDMETHOD_(void, Clear)
  (ID2D1RenderTarget *This, __in_opt CONST D2D1_COLOR_F *clearColor) PURE;
  ...
  (ID2D1RenderTarget *This, FLOAT dpiX, FLOAT dpiY) PURE;

  STDMETHOD_(void, GetDpi)
  (ID2D1RenderTarget *This, __out FLOAT *dpiX, __out FLOAT *dpiY) PURE;
  ...
  STDMETHOD_(BOOL, IsSupported)
  (ID2D1RenderTarget *This,
   __in CONST D2D1_RENDER_TARGET_PROPERTIES *renderTargetProperties) PURE;
...
#define ID2D1RenderTarget_CreateBitmap(This, size, srcData, pitch,             \
                                       bitmapProperties, bitmap)               \
  ((This)->lpVtbl->CreateBitmap(This, size, srcData, pitch, bitmapProperties,  \
                                bitmap))

#define ID2D1RenderTarget_CreateBitmapFromWicBitmap(This, wicBitmapSource,     \
                                                    bitmapProperties, bitmap)  \
  ((This)->lpVtbl->CreateBitmapFromWicBitmap(This, wicBitmapSource,            \
                                             bitmapProperties, bitmap))

#define ID2D1RenderTarget_CreateSharedBitmap(This, riid, data,                 \
                                             bitmapProperties, bitmap)         \
  ((This)->lpVtbl->CreateSharedBitmap(This, riid, data, bitmapProperties,      \
                                      bitmap))

#define ID2D1RenderTarget_CreateBitmapBrush(                                   \
    This, bitmap, bitmapBrushProperties, brushProperties, bitmapBrush)         \
  ((This)->lpVtbl->CreateBitmapBrush(This, bitmap, bitmapBrushProperties,      \
                                     brushProperties, bitmapBrush))

#define ID2D1RenderTarget_CreateSolidColorBrush(This, color, brushProperties,  \
                                                solidColorBrush)               \
  ((This)->lpVtbl->CreateSolidColorBrush(This, color, brushProperties,         \
                                         solidColorBrush))

#define ID2D1RenderTarget_CreateGradientStopCollection(                        \
    This, gradientStops, gradientStopsCount, colorInterpolationGamma,          \
    extendMode, gradientStopCollection)                                        \
  ((This)->lpVtbl->CreateGradientStopCollection(                               \
      This, gradientStops, gradientStopsCount, colorInterpolationGamma,        \
      extendMode, gradientStopCollection))

#define ID2D1RenderTarget_CreateLinearGradientBrush(                           \
    This, linearGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, linearGradientBrush)                               \
  ((This)->lpVtbl->CreateLinearGradientBrush(                                  \
      This, linearGradientBrushProperties, brushProperties,                    \
      gradientStopCollection, linearGradientBrush))

#define ID2D1RenderTarget_CreateRadialGradientBrush(                           \
    This, radialGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, radialGradientBrush)                               \
  ((This)->lpVtbl->CreateRadialGradientBrush(                                  \
      This, radialGradientBrushProperties, brushProperties,                    \
      gradientStopCollection, radialGradientBrush))

#define ID2D1RenderTarget_CreateCompatibleRenderTarget(                        \
    This, desiredSize, desiredPixelSize, desiredFormat, options,               \
    bitmapRenderTarget)                                                        \
  ((This)->lpVtbl->CreateCompatibleRenderTarget(                               \
      This, desiredSize, desiredPixelSize, desiredFormat, options,             \
      bitmapRenderTarget))
...
#define ID2D1RenderTarget_DrawLine(This, point0, point1, brush, strokeWidth,   \
                                   strokeStyle)                                \
  ((This)->lpVtbl->DrawLine(This, point0, point1, brush, strokeWidth,          \
                            strokeStyle))

#define ID2D1RenderTarget_DrawRectangle(This, rect, brush, strokeWidth,        \
                                        strokeStyle)                           \
...
#define ID2D1RenderTarget_DrawRoundedRectangle(This, roundedRect, brush,       \
...
#define ID2D1RenderTarget_DrawEllipse(This, ellipse, brush, strokeWidth,       \
                                      strokeStyle)                             \
...
#define ID2D1RenderTarget_DrawGeometry(This, geometry, brush, strokeWidth,     \
                                       strokeStyle)                            \
  ((This)->lpVtbl->DrawGeometry(This, geometry, brush, strokeWidth,            \
                                strokeStyle))
...
#define ID2D1RenderTarget_FillOpacityMask(                                     \
    This, opacityMask, brush, content, destinationRectangle, sourceRectangle)  \
  ((This)->lpVtbl->FillOpacityMask(This, opacityMask, brush, content,          \
                                   destinationRectangle, sourceRectangle))

#define ID2D1RenderTarget_DrawBitmap(This, bitmap, destinationRectangle,       \
                                     opacity, interpolationMode,               \
                                     sourceRectangle)                          \
  ((This)->lpVtbl->DrawBitmap(This, bitmap, destinationRectangle, opacity,     \
                              interpolationMode, sourceRectangle))

#define ID2D1RenderTarget_DrawText(This, string, stringLength, textFormat,     \
                                   layoutRect, defaultForegroundBrush,         \
                                   options, measuringMode)                     \
  ((This)->lpVtbl->DrawText(This, string, stringLength, textFormat,            \
                            layoutRect, defaultForegroundBrush, options,       \
                            measuringMode))

#define ID2D1RenderTarget_DrawTextLayout(This, origin, textLayout,             \
                                         defaultForegroundBrush, options)      \
  ((This)->lpVtbl->DrawTextLayout(This, origin, textLayout,                    \
                                  defaultForegroundBrush, options))

#define ID2D1RenderTarget_DrawGlyphRun(This, baselineOrigin, glyphRun,         \
...
typedef struct ID2D1BitmapRenderTargetVtbl {
  ...
  STDMETHOD(GetBitmap)
  (ID2D1BitmapRenderTarget *This, __deref_out ID2D1Bitmap **bitmap) PURE;
...
#define ID2D1BitmapRenderTarget_CreateBitmap(This, size, srcData, pitch,       \
                                             bitmapProperties, bitmap)         \
  ((This)->lpVtbl->Base.CreateBitmap((ID2D1RenderTarget *)This, size, srcData, \
                                     pitch, bitmapProperties, bitmap))

#define ID2D1BitmapRenderTarget_CreateBitmapFromWicBitmap(                     \
    This, wicBitmapSource, bitmapProperties, bitmap)                           \
  ((This)->lpVtbl->Base.CreateBitmapFromWicBitmap(                             \
      (ID2D1RenderTarget *)This, wicBitmapSource, bitmapProperties, bitmap))

#define ID2D1BitmapRenderTarget_CreateSharedBitmap(This, riid, data,           \
                                                   bitmapProperties, bitmap)   \
  ((This)->lpVtbl->Base.CreateSharedBitmap((ID2D1RenderTarget *)This, riid,    \
                                           data, bitmapProperties, bitmap))

#define ID2D1BitmapRenderTarget_CreateBitmapBrush(                             \
    This, bitmap, bitmapBrushProperties, brushProperties, bitmapBrush)         \
  ((This)->lpVtbl->Base.CreateBitmapBrush((ID2D1RenderTarget *)This, bitmap,   \
                                          bitmapBrushProperties,               \
                                          brushProperties, bitmapBrush))

#define ID2D1BitmapRenderTarget_CreateSolidColorBrush(                         \
    This, color, brushProperties, solidColorBrush)                             \
  ((This)->lpVtbl->Base.CreateSolidColorBrush(                                 \
      (ID2D1RenderTarget *)This, color, brushProperties, solidColorBrush))

#define ID2D1BitmapRenderTarget_CreateGradientStopCollection(                  \
    This, gradientStops, gradientStopsCount, colorInterpolationGamma,          \
    extendMode, gradientStopCollection)                                        \
  ((This)->lpVtbl->Base.CreateGradientStopCollection(                          \
      (ID2D1RenderTarget *)This, gradientStops, gradientStopsCount,            \
      colorInterpolationGamma, extendMode, gradientStopCollection))

#define ID2D1BitmapRenderTarget_CreateLinearGradientBrush(                     \
    This, linearGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, linearGradientBrush)                               \
  ((This)->lpVtbl->Base.CreateLinearGradientBrush(                             \
      (ID2D1RenderTarget *)This, linearGradientBrushProperties,                \
      brushProperties, gradientStopCollection, linearGradientBrush))

#define ID2D1BitmapRenderTarget_CreateRadialGradientBrush(                     \
    This, radialGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, radialGradientBrush)                               \
  ((This)->lpVtbl->Base.CreateRadialGradientBrush(                             \
      (ID2D1RenderTarget *)This, radialGradientBrushProperties,                \
      brushProperties, gradientStopCollection, radialGradientBrush))

#define ID2D1BitmapRenderTarget_CreateCompatibleRenderTarget(                  \
    This, desiredSize, desiredPixelSize, desiredFormat, options,               \
    bitmapRenderTarget)                                                        \
  ((This)->lpVtbl->Base.CreateCompatibleRenderTarget(                          \
      (ID2D1RenderTarget *)This, desiredSize, desiredPixelSize, desiredFormat, \
...
#define ID2D1BitmapRenderTarget_DrawLine(This, point0, point1, brush,          \
                                         strokeWidth, strokeStyle)             \
  ((This)->lpVtbl->Base.DrawLine((ID2D1RenderTarget *)This, point0, point1,    \
                                 brush, strokeWidth, strokeStyle))

#define ID2D1BitmapRenderTarget_DrawRectangle(This, rect, brush, strokeWidth,  \
                                              strokeStyle)                     \
...
#define ID2D1BitmapRenderTarget_DrawRoundedRectangle(This, roundedRect, brush, \
                                                     strokeWidth, strokeStyle) \
  ((This)->lpVtbl->Base.DrawRoundedRectangle((ID2D1RenderTarget *)This,        \
                                             roundedRect, brush, strokeWidth,  \
                                             strokeStyle))

#define ID2D1BitmapRenderTarget_FillRoundedRectangle(This, roundedRect, brush) \
  ((This)->lpVtbl->Base.FillRoundedRectangle((ID2D1RenderTarget *)This,        \
                                             roundedRect, brush))

#define ID2D1BitmapRenderTarget_DrawEllipse(This, ellipse, brush, strokeWidth, \
                                            strokeStyle)                       \
...
#define ID2D1BitmapRenderTarget_DrawGeometry(This, geometry, brush,            \
                                             strokeWidth, strokeStyle)         \
  ((This)->lpVtbl->Base.DrawGeometry((ID2D1RenderTarget *)This, geometry,      \
                                     brush, strokeWidth, strokeStyle))

#define ID2D1BitmapRenderTarget_FillGeometry(This, geometry, brush,            \
                                             opacityBrush)                     \
  ((This)->lpVtbl->Base.FillGeometry((ID2D1RenderTarget *)This, geometry,      \
                                     brush, opacityBrush))

#define ID2D1BitmapRenderTarget_FillMesh(This, mesh, brush)                    \
  ((This)->lpVtbl->Base.FillMesh((ID2D1RenderTarget *)This, mesh, brush))

#define ID2D1BitmapRenderTarget_FillOpacityMask(                               \
    This, opacityMask, brush, content, destinationRectangle, sourceRectangle)  \
  ((This)->lpVtbl->Base.FillOpacityMask(                                       \
      (ID2D1RenderTarget *)This, opacityMask, brush, content,                  \
      destinationRectangle, sourceRectangle))

#define ID2D1BitmapRenderTarget_DrawBitmap(This, bitmap, destinationRectangle, \
                                           opacity, interpolationMode,         \
                                           sourceRectangle)                    \
  ((This)->lpVtbl->Base.DrawBitmap((ID2D1RenderTarget *)This, bitmap,          \
                                   destinationRectangle, opacity,              \
                                   interpolationMode, sourceRectangle))

#define ID2D1BitmapRenderTarget_DrawText(                                      \
    This, string, stringLength, textFormat, layoutRect,                        \
    defaultForegroundBrush, options, measuringMode)                            \
  ((This)->lpVtbl->Base.DrawText(                                              \
      (ID2D1RenderTarget *)This, string, stringLength, textFormat, layoutRect, \
      defaultForegroundBrush, options, measuringMode))

#define ID2D1BitmapRenderTarget_DrawTextLayout(                                \
    This, origin, textLayout, defaultForegroundBrush, options)                 \
  ((This)->lpVtbl->Base.DrawTextLayout((ID2D1RenderTarget *)This, origin,      \
                                       textLayout, defaultForegroundBrush,     \
                                       options))

#define ID2D1BitmapRenderTarget_DrawGlyphRun(This, baselineOrigin, glyphRun,   \
                                             foregroundBrush, measuringMode)   \
  ((This)->lpVtbl->Base.DrawGlyphRun((ID2D1RenderTarget *)This,                \
                                     baselineOrigin, glyphRun,                 \
                                     foregroundBrush, measuringMode))
...
#define ID2D1BitmapRenderTarget_SetAntialiasMode(This, antialiasMode)          \
  ((This)->lpVtbl->Base.SetAntialiasMode((ID2D1RenderTarget *)This,            \
                                         antialiasMode))
...
#define ID2D1BitmapRenderTarget_SetTextAntialiasMode(This, textAntialiasMode)  \
  ((This)->lpVtbl->Base.SetTextAntialiasMode((ID2D1RenderTarget *)This,        \
                                             textAntialiasMode))

#define ID2D1BitmapRenderTarget_GetTextAntialiasMode(This)                     \
  ((This)->lpVtbl->Base.GetTextAntialiasMode((ID2D1RenderTarget *)This))

#define ID2D1BitmapRenderTarget_SetTextRenderingParams(This,                   \
                                                       textRenderingParams)    \
  ((This)->lpVtbl->Base.SetTextRenderingParams((ID2D1RenderTarget *)This,      \
                                               textRenderingParams))

#define ID2D1BitmapRenderTarget_GetTextRenderingParams(This,                   \
                                                       textRenderingParams)    \
  ((This)->lpVtbl->Base.GetTextRenderingParams((ID2D1RenderTarget *)This,      \
                                               textRenderingParams))
...
#define ID2D1BitmapRenderTarget_SaveDrawingState(This, drawingStateBlock)      \
  ((This)->lpVtbl->Base.SaveDrawingState((ID2D1RenderTarget *)This,            \
                                         drawingStateBlock))

#define ID2D1BitmapRenderTarget_RestoreDrawingState(This, drawingStateBlock)   \
  ((This)->lpVtbl->Base.RestoreDrawingState((ID2D1RenderTarget *)This,         \
                                            drawingStateBlock))

#define ID2D1BitmapRenderTarget_PushAxisAlignedClip(This, clipRect,            \
                                                    antialiasMode)             \
  ((This)->lpVtbl->Base.PushAxisAlignedClip((ID2D1RenderTarget *)This,         \
                                            clipRect, antialiasMode))
...
#define ID2D1BitmapRenderTarget_IsSupported(This, renderTargetProperties)      \
  ((This)->lpVtbl->Base.IsSupported((ID2D1RenderTarget *)This,                 \
                                    renderTargetProperties))
...
typedef struct ID2D1HwndRenderTargetVtbl {
  ...
  (ID2D1HwndRenderTarget *This) PURE;

  STDMETHOD(Resize)
  (ID2D1HwndRenderTarget *This, __in CONST D2D1_SIZE_U *pixelSize) PURE;
...
#define ID2D1HwndRenderTarget_CreateBitmap(This, size, srcData, pitch,         \
                                           bitmapProperties, bitmap)           \
  ((This)->lpVtbl->Base.CreateBitmap((ID2D1RenderTarget *)This, size, srcData, \
                                     pitch, bitmapProperties, bitmap))

#define ID2D1HwndRenderTarget_CreateBitmapFromWicBitmap(                       \
    This, wicBitmapSource, bitmapProperties, bitmap)                           \
  ((This)->lpVtbl->Base.CreateBitmapFromWicBitmap(                             \
      (ID2D1RenderTarget *)This, wicBitmapSource, bitmapProperties, bitmap))

#define ID2D1HwndRenderTarget_CreateSharedBitmap(This, riid, data,             \
                                                 bitmapProperties, bitmap)     \
  ((This)->lpVtbl->Base.CreateSharedBitmap((ID2D1RenderTarget *)This, riid,    \
                                           data, bitmapProperties, bitmap))

#define ID2D1HwndRenderTarget_CreateBitmapBrush(                               \
    This, bitmap, bitmapBrushProperties, brushProperties, bitmapBrush)         \
  ((This)->lpVtbl->Base.CreateBitmapBrush((ID2D1RenderTarget *)This, bitmap,   \
                                          bitmapBrushProperties,               \
                                          brushProperties, bitmapBrush))

#define ID2D1HwndRenderTarget_CreateSolidColorBrush(                           \
    This, color, brushProperties, solidColorBrush)                             \
  ((This)->lpVtbl->Base.CreateSolidColorBrush(                                 \
      (ID2D1RenderTarget *)This, color, brushProperties, solidColorBrush))
...
#define ID2D1HwndRenderTarget_CreateLinearGradientBrush(                       \
    This, linearGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, linearGradientBrush)                               \
  ((This)->lpVtbl->Base.CreateLinearGradientBrush(                             \
      (ID2D1RenderTarget *)This, linearGradientBrushProperties,                \
...
#define ID2D1HwndRenderTarget_CreateRadialGradientBrush(                       \
    This, radialGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, radialGradientBrush)                               \
  ((This)->lpVtbl->Base.CreateRadialGradientBrush(                             \
      (ID2D1RenderTarget *)This, radialGradientBrushProperties,                \
...
#define ID2D1HwndRenderTarget_CreateCompatibleRenderTarget(                    \
    This, desiredSize, desiredPixelSize, desiredFormat, options,               \
    bitmapRenderTarget)                                                        \
  ((This)->lpVtbl->Base.CreateCompatibleRenderTarget(                          \
      (ID2D1RenderTarget *)This, desiredSize, desiredPixelSize, desiredFormat, \
...
#define ID2D1HwndRenderTarget_DrawLine(This, point0, point1, brush,            \
                                       strokeWidth, strokeStyle)               \
  ((This)->lpVtbl->Base.DrawLine((ID2D1RenderTarget *)This, point0, point1,    \
                                 brush, strokeWidth, strokeStyle))

#define ID2D1HwndRenderTarget_DrawRectangle(This, rect, brush, strokeWidth,    \
                                            strokeStyle)                       \
  ((This)->lpVtbl->Base.DrawRectangle((ID2D1RenderTarget *)This, rect, brush,  \
                                      strokeWidth, strokeStyle))
...
#define ID2D1HwndRenderTarget_DrawRoundedRectangle(This, roundedRect, brush,   \
                                                   strokeWidth, strokeStyle)   \
  ((This)->lpVtbl->Base.DrawRoundedRectangle((ID2D1RenderTarget *)This,        \
                                             roundedRect, brush, strokeWidth,  \
                                             strokeStyle))
...
#define ID2D1HwndRenderTarget_DrawEllipse(This, ellipse, brush, strokeWidth,   \
                                          strokeStyle)                         \
  ((This)->lpVtbl->Base.DrawEllipse((ID2D1RenderTarget *)This, ellipse, brush, \
                                    strokeWidth, strokeStyle))
...
#define ID2D1HwndRenderTarget_DrawGeometry(This, geometry, brush, strokeWidth, \
                                           strokeStyle)                        \
  ((This)->lpVtbl->Base.DrawGeometry((ID2D1RenderTarget *)This, geometry,      \
                                     brush, strokeWidth, strokeStyle))

#define ID2D1HwndRenderTarget_FillGeometry(This, geometry, brush,              \
                                           opacityBrush)                       \
  ((This)->lpVtbl->Base.FillGeometry((ID2D1RenderTarget *)This, geometry,      \
                                     brush, opacityBrush))
...
#define ID2D1HwndRenderTarget_FillOpacityMask(                                 \
    This, opacityMask, brush, content, destinationRectangle, sourceRectangle)  \
  ((This)->lpVtbl->Base.FillOpacityMask(                                       \
      (ID2D1RenderTarget *)This, opacityMask, brush, content,                  \
      destinationRectangle, sourceRectangle))

#define ID2D1HwndRenderTarget_DrawBitmap(This, bitmap, destinationRectangle,   \
                                         opacity, interpolationMode,           \
                                         sourceRectangle)                      \
  ((This)->lpVtbl->Base.DrawBitmap((ID2D1RenderTarget *)This, bitmap,          \
                                   destinationRectangle, opacity,              \
...
#define ID2D1HwndRenderTarget_DrawText(This, string, stringLength, textFormat, \
                                       layoutRect, defaultForegroundBrush,     \
                                       options, measuringMode)                 \
  ((This)->lpVtbl->Base.DrawText(                                              \
      (ID2D1RenderTarget *)This, string, stringLength, textFormat, layoutRect, \
...
#define ID2D1HwndRenderTarget_DrawTextLayout(This, origin, textLayout,         \
                                             defaultForegroundBrush, options)  \
  ((This)->lpVtbl->Base.DrawTextLayout((ID2D1RenderTarget *)This, origin,      \
                                       textLayout, defaultForegroundBrush,     \
                                       options))

#define ID2D1HwndRenderTarget_DrawGlyphRun(This, baselineOrigin, glyphRun,     \
                                           foregroundBrush, measuringMode)     \
  ((This)->lpVtbl->Base.DrawGlyphRun((ID2D1RenderTarget *)This,                \
                                     baselineOrigin, glyphRun,                 \
                                     foregroundBrush, measuringMode))
...
#define ID2D1HwndRenderTarget_SetTextRenderingParams(This,                     \
                                                     textRenderingParams)      \
  ((This)->lpVtbl->Base.SetTextRenderingParams((ID2D1RenderTarget *)This,      \
                                               textRenderingParams))

#define ID2D1HwndRenderTarget_GetTextRenderingParams(This,                     \
                                                     textRenderingParams)      \
  ((This)->lpVtbl->Base.GetTextRenderingParams((ID2D1RenderTarget *)This,      \
                                               textRenderingParams))
...
#define ID2D1HwndRenderTarget_PushAxisAlignedClip(This, clipRect,              \
                                                  antialiasMode)               \
  ((This)->lpVtbl->Base.PushAxisAlignedClip((ID2D1RenderTarget *)This,         \
                                            clipRect, antialiasMode))
...
typedef struct ID2D1GdiInteropRenderTargetVtbl {
  ...
  STDMETHOD(GetDC)
  (ID2D1GdiInteropRenderTarget *This, D2D1_DC_INITIALIZE_MODE mode,
   __out HDC *hdc) PURE;

  STDMETHOD(ReleaseDC)
  (ID2D1GdiInteropRenderTarget *This, __in_opt CONST RECT *update) PURE;
...
typedef struct ID2D1DCRenderTargetVtbl {
  ...
  STDMETHOD(BindDC)
  (ID2D1DCRenderTarget *This, __in CONST HDC hDC,
   __in CONST RECT *pSubRect) PURE;
...
#define ID2D1DCRenderTarget_CreateBitmap(This, size, srcData, pitch,           \
                                         bitmapProperties, bitmap)             \
  ((This)->lpVtbl->Base.CreateBitmap((ID2D1RenderTarget *)This, size, srcData, \
                                     pitch, bitmapProperties, bitmap))

#define ID2D1DCRenderTarget_CreateBitmapFromWicBitmap(                         \
    This, wicBitmapSource, bitmapProperties, bitmap)                           \
  ((This)->lpVtbl->Base.CreateBitmapFromWicBitmap(                             \
      (ID2D1RenderTarget *)This, wicBitmapSource, bitmapProperties, bitmap))

#define ID2D1DCRenderTarget_CreateSharedBitmap(This, riid, data,               \
                                               bitmapProperties, bitmap)       \
  ((This)->lpVtbl->Base.CreateSharedBitmap((ID2D1RenderTarget *)This, riid,    \
                                           data, bitmapProperties, bitmap))

#define ID2D1DCRenderTarget_CreateBitmapBrush(                                 \
    This, bitmap, bitmapBrushProperties, brushProperties, bitmapBrush)         \
  ((This)->lpVtbl->Base.CreateBitmapBrush((ID2D1RenderTarget *)This, bitmap,   \
                                          bitmapBrushProperties,               \
                                          brushProperties, bitmapBrush))
...
#define ID2D1DCRenderTarget_CreateRadialGradientBrush(                         \
    This, radialGradientBrushProperties, brushProperties,                      \
    gradientStopCollection, radialGradientBrush)                               \
  ((This)->lpVtbl->Base.CreateRadialGradientBrush(                             \
      (ID2D1RenderTarget *)This, radialGradientBrushProperties,                \
...
#define ID2D1DCRenderTarget_CreateCompatibleRenderTarget(                      \
    This, desiredSize, desiredPixelSize, desiredFormat, options,               \
    bitmapRenderTarget)                                                        \
  ((This)->lpVtbl->Base.CreateCompatibleRenderTarget(                          \
      (ID2D1RenderTarget *)This, desiredSize, desiredPixelSize, desiredFormat, \
...
#define ID2D1DCRenderTarget_DrawLine(This, point0, point1, brush, strokeWidth, \
                                     strokeStyle)                              \
  ((This)->lpVtbl->Base.DrawLine((ID2D1RenderTarget *)This, point0, point1,    \
                                 brush, strokeWidth, strokeStyle))

#define ID2D1DCRenderTarget_DrawRectangle(This, rect, brush, strokeWidth,      \
                                          strokeStyle)                         \
  ((This)->lpVtbl->Base.DrawRectangle((ID2D1RenderTarget *)This, rect, brush,  \
                                      strokeWidth, strokeStyle))
...
#define ID2D1DCRenderTarget_DrawRoundedRectangle(This, roundedRect, brush,     \
                                                 strokeWidth, strokeStyle)     \
  ((This)->lpVtbl->Base.DrawRoundedRectangle((ID2D1RenderTarget *)This,        \
                                             roundedRect, brush, strokeWidth,  \
                                             strokeStyle))
...
#define ID2D1DCRenderTarget_DrawEllipse(This, ellipse, brush, strokeWidth,     \
                                        strokeStyle)                           \
  ((This)->lpVtbl->Base.DrawEllipse((ID2D1RenderTarget *)This, ellipse, brush, \
                                    strokeWidth, strokeStyle))
...
#define ID2D1DCRenderTarget_DrawGeometry(This, geometry, brush, strokeWidth,   \
                                         strokeStyle)                          \
  ((This)->lpVtbl->Base.DrawGeometry((ID2D1RenderTarget *)This, geometry,      \
                                     brush, strokeWidth, strokeStyle))
...
#define ID2D1DCRenderTarget_FillOpacityMask(                                   \
    This, opacityMask, brush, content, destinationRectangle, sourceRectangle)  \
  ((This)->lpVtbl->Base.FillOpacityMask(                                       \
      (ID2D1RenderTarget *)This, opacityMask, brush, content,                  \
      destinationRectangle, sourceRectangle))

#define ID2D1DCRenderTarget_DrawBitmap(This, bitmap, destinationRectangle,     \
                                       opacity, interpolationMode,             \
                                       sourceRectangle)                        \
  ((This)->lpVtbl->Base.DrawBitmap((ID2D1RenderTarget *)This, bitmap,          \
                                   destinationRectangle, opacity,              \
...
#define ID2D1DCRenderTarget_DrawText(This, string, stringLength, textFormat,   \
                                     layoutRect, defaultForegroundBrush,       \
                                     options, measuringMode)                   \
  ((This)->lpVtbl->Base.DrawText(                                              \
      (ID2D1RenderTarget *)This, string, stringLength, textFormat, layoutRect, \
...
#define ID2D1DCRenderTarget_DrawTextLayout(This, origin, textLayout,           \
                                           defaultForegroundBrush, options)    \
  ((This)->lpVtbl->Base.DrawTextLayout((ID2D1RenderTarget *)This, origin,      \
                                       textLayout, defaultForegroundBrush,     \
                                       options))

#define ID2D1DCRenderTarget_DrawGlyphRun(This, baselineOrigin, glyphRun,       \
                                         foregroundBrush, measuringMode)       \
  ((This)->lpVtbl->Base.DrawGlyphRun((ID2D1RenderTarget *)This,                \
                                     baselineOrigin, glyphRun,                 \
                                     foregroundBrush, measuringMode))
...
typedef struct ID2D1FactoryVtbl {
  ...
  (ID2D1Factory *This, __out FLOAT *dpiX, __out FLOAT *dpiY) PURE;

  STDMETHOD(CreateRectangleGeometry)
  (ID2D1Factory *This, __in CONST D2D1_RECT_F *rectangle,
   __deref_out ID2D1RectangleGeometry **rectangleGeometry) PURE;

  STDMETHOD(CreateRoundedRectangleGeometry)
  (ID2D1Factory *This, __in CONST D2D1_ROUNDED_RECT *roundedRectangle,
   __deref_out ID2D1RoundedRectangleGeometry **roundedRectangleGeometry) PURE;

  STDMETHOD(CreateEllipseGeometry)
  (ID2D1Factory *This, __in CONST D2D1_ELLIPSE *ellipse,
   __deref_out ID2D1EllipseGeometry **ellipseGeometry) PURE;

  STDMETHOD(CreateGeometryGroup)
  (ID2D1Factory *This, D2D1_FILL_MODE fillMode,
   __in_ecount(geometriesCount) ID2D1Geometry **geometries,
   UINT geometriesCount, __deref_out ID2D1GeometryGroup **geometryGroup) PURE;

  STDMETHOD(CreateTransformedGeometry)
  (ID2D1Factory *This, __in ID2D1Geometry *sourceGeometry,
   __in CONST D2D1_MATRIX_3X2_F *transform,
   __deref_out ID2D1TransformedGeometry **transformedGeometry) PURE;

  STDMETHOD(CreatePathGeometry)
  (ID2D1Factory *This, __deref_out ID2D1PathGeometry **pathGeometry) PURE;
  ...
  STDMETHOD(CreateWicBitmapRenderTarget)
  (ID2D1Factory *This, __in IWICBitmap *target,
   __in CONST D2D1_RENDER_TARGET_PROPERTIES *renderTargetProperties,
   __deref_out ID2D1RenderTarget **renderTarget) PURE;
  ...
  STDMETHOD(CreateDxgiSurfaceRenderTarget)
  (ID2D1Factory *This, __in IDXGISurface *dxgiSurface,
   __in CONST D2D1_RENDER_TARGET_PROPERTIES *renderTargetProperties,
   __deref_out ID2D1RenderTarget **renderTarget) PURE;
...
#define ID2D1Factory_CreateRectangleGeometry(This, rectangle,                  \
                                             rectangleGeometry)                \
  ((This)->lpVtbl->CreateRectangleGeometry(This, rectangle, rectangleGeometry))

#define ID2D1Factory_CreateRoundedRectangleGeometry(This, roundedRectangle,    \
                                                    roundedRectangleGeometry)  \
  ((This)->lpVtbl->CreateRoundedRectangleGeometry(This, roundedRectangle,      \
                                                  roundedRectangleGeometry))
...
#define ID2D1Factory_CreateGeometryGroup(This, fillMode, geometries,           \
                                         geometriesCount, geometryGroup)       \
  ((This)->lpVtbl->CreateGeometryGroup(This, fillMode, geometries,             \
                                       geometriesCount, geometryGroup))

#define ID2D1Factory_CreateTransformedGeometry(This, sourceGeometry,           \
...
#define ID2D1Factory_CreateStrokeStyle(This, strokeStyleProperties, dashes,    \
                                       dashesCount, strokeStyle)               \
  ((This)->lpVtbl->CreateStrokeStyle(This, strokeStyleProperties, dashes,      \
                                     dashesCount, strokeStyle))

#define ID2D1Factory_CreateDrawingStateBlock(                                  \
    This, drawingStateDescription, textRenderingParams, drawingStateBlock)     \
  ((This)->lpVtbl->CreateDrawingStateBlock(                                    \
      This, drawingStateDescription, textRenderingParams, drawingStateBlock))

#define ID2D1Factory_CreateWicBitmapRenderTarget(                              \
    This, target, renderTargetProperties, renderTarget)                        \
  ((This)->lpVtbl->CreateWicBitmapRenderTarget(                                \
      This, target, renderTargetProperties, renderTarget))

#define ID2D1Factory_CreateHwndRenderTarget(This, renderTargetProperties,      \
                                            hwndRenderTargetProperties,        \
                                            hwndRenderTarget)                  \
  ((This)->lpVtbl->CreateHwndRenderTarget(This, renderTargetProperties,        \
                                          hwndRenderTargetProperties,          \
                                          hwndRenderTarget))

#define ID2D1Factory_CreateDxgiSurfaceRenderTarget(                            \
    This, dxgiSurface, renderTargetProperties, renderTarget)                   \
  ((This)->lpVtbl->CreateDxgiSurfaceRenderTarget(                              \
      This, dxgiSurface, renderTargetProperties, renderTarget))

#define ID2D1Factory_CreateDCRenderTarget(This, renderTargetProperties,        \
                                          dcRenderTarget)                      \
  ((This)->lpVtbl->CreateDCRenderTarget(This, renderTargetProperties,          \
                                        dcRenderTarget))
...
// consistent also, this must be 'C' callable.
//
HRESULT WINAPI
D2D1CreateFactory(__in D2D1_FACTORY_TYPE factoryType, __in REFIID riid,
                  __in_opt CONST D2D1_FACTORY_OPTIONS *pFactoryOptions,
                  __out void **ppIFactory);

void WINAPI D2D1MakeRotateMatrix(__in FLOAT angle, __in D2D1_POINT_2F center,
                                 __out D2D1_MATRIX_3X2_F *matrix);

void WINAPI D2D1MakeSkewMatrix(__in FLOAT angleX, __in FLOAT angleY,
                               __in D2D1_POINT_2F center,
                               __out D2D1_MATRIX_3X2_F *matrix);

BOOL WINAPI D2D1IsMatrixInvertible(__in CONST D2D1_MATRIX_3X2_F *matrix);

BOOL WINAPI D2D1InvertMatrix(__inout D2D1_MATRIX_3X2_F *matrix);
...
inline HRESULT D2D1CreateFactory(__in D2D1_FACTORY_TYPE factoryType,
                                 __in REFIID riid, __out void **factory) {
  return D2D1CreateFactory(factoryType, riid, NULL, factory);
}

template <class Factory>
HRESULT D2D1CreateFactory(__in D2D1_FACTORY_TYPE factoryType,
                          __out Factory **factory) {
  return D2D1CreateFactory(factoryType, __uuidof(Factory),
...
HRESULT D2D1CreateFactory(__in D2D1_FACTORY_TYPE factoryType,
  ...
  return D2D1CreateFactory(factoryType, __uuidof(Factory), &factoryOptions,
