...
// default audio format ranges supported, applies to
// XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.pFormat
#define XAPOBASE_DEFAULT_FORMAT_TAG                                            \
  WAVE_FORMAT_IEEE_FLOAT // 32-bit float only, applies to
                         // WAVEFORMATEX.wFormatTag or
                         // WAVEFORMATEXTENSIBLE.SubFormat when used
#define XAPOBASE_DEFAULT_FORMAT_MIN_CHANNELS                                   \
  XAPO_MIN_CHANNELS // minimum channel count, applies to WAVEFORMATEX.nChannels
#define XAPOBASE_DEFAULT_FORMAT_MAX_CHANNELS                                   \
  XAPO_MAX_CHANNELS // maximum channel count, applies to WAVEFORMATEX.nChannels
#define XAPOBASE_DEFAULT_FORMAT_MIN_FRAMERATE                                  \
  XAPO_MIN_FRAMERATE // minimum framerate, applies to
                     // WAVEFORMATEX.nSamplesPerSec
#define XAPOBASE_DEFAULT_FORMAT_MAX_FRAMERATE                                  \
  XAPO_MAX_FRAMERATE // maximum framerate, applies to
                     // WAVEFORMATEX.nSamplesPerSec
#define XAPOBASE_DEFAULT_FORMAT_BITSPERSAMPLE                                  \
  32 // 32-bit float only, applies to WAVEFORMATEX.wBitsPerSample and
     // WAVEFORMATEXTENSIBLE.wValidBitsPerSample when used

// default XAPO property flags supported, applies to
// XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS
#define XAPOBASE_DEFAULT_FLAG                                                  \
...
// default number of input and output buffers supported, applies to
// XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS
...
//--------------<D-A-T-A---T-Y-P-E-S>---------------------------------------//
#pragma pack(push,                                                             \
             8) // set packing alignment to ensure consistency across arbitrary
                // build environments, and ensure synchronization variables used
                // by Interlocked functionality are correctly aligned
...
private:
  const XAPO_REGISTRATION_PROPERTIES
      *m_pRegistrationProperties; // pointer to registration properties of the
                                  // XAPO, set via constructor

  void *m_pfnMatrixMixFunction; // optimal matrix function pointer, used for
                                // thru processing
  FLOAT32 *m_pfl32MatrixCoefficients; // matrix coefficient table, used for thru
                                      // processing
  UINT32 m_nSrcFormatType; // input format type, used for thru processing
  BOOL
      m_fIsScalarMatrix; // TRUE if m_pfl32MatrixCoefficients is diagonal matrix
                         // with all main diagonal entries equal, i.e.
                         // m_pfnMatrixMixFunction only used for type conversion
                         // (no channel conversion), used for thru processing
  BOOL m_fIsLocked;      // TRUE if XAPO locked via CXAPOBase.LockForProcess

protected:
  LONG m_lReferenceCount; // COM reference count, must be aligned for atomic
                          // operations
  ...
  //  fOverwrite - [in]     TRUE to overwrite pFormat if audio format
  //  unsupported
  //
  // RETURN VALUE:
  //  COM error code, including:
  //    S_OK                      - audio format supported, pFormat left
  //    untouched XAPO_E_FORMAT_UNSUPPORTED - audio format unsupported, pFormat
  //    overwritten with nearest audio format supported if fOverwrite TRUE
  //    E_INVALIDARG              - audio format invalid, pFormat left untouched
  ////
  virtual HRESULT ValidateFormatDefault(__inout WAVEFORMATEX *pFormat,
                                        BOOL fOverwrite);
  ...
  //  pRequestedFormat - [in/out] audio format to examine, must be
  //  WAVEFORMATEXTENSIBLE if fOverwrite TRUE fOverwrite       - [in]     TRUE
  //  to overwrite pRequestedFormat if input/output configuration unsupported
  //
  // RETURN VALUE:
  //  COM error code, including:
  //    S_OK                      - input/output configuration supported,
  //    pRequestedFormat left untouched XAPO_E_FORMAT_UNSUPPORTED - input/output
  //    configuration unsupported, pRequestedFormat overwritten with nearest
  //    audio format supported if fOverwrite TRUE E_INVALIDARG              -
  //    either audio format invalid, pRequestedFormat left untouched
  ////
  HRESULT ValidateFormatPair(const WAVEFORMATEX *pSupportedFormat,
                             __inout WAVEFORMATEX *pRequestedFormat,
                             BOOL fOverwrite);
  ...
  //
  // PARAMETERS:
  //  pInputBuffer       - [in]  input buffer, format may be INT8, INT16, INT20
  //  (contained in 24 or 32 bits), INT24 (contained in 24 or 32 bits), INT32,
  //  or FLOAT32 pOutputBuffer      - [out] output buffer, format must be
  //  FLOAT32 FrameCount         - [in]  number of frames to process
  //  InputChannelCount  - [in]  number of input channels
  //  OutputChannelCount - [in]  number of output channels
  //  MixWithOutput      - [in]  TRUE to mix with output, FALSE to overwrite
  //  output
  //
  // RETURN VALUE:
  //  void
  ////
  void ProcessThru(__in void *pInputBuffer, __inout FLOAT32 *pOutputBuffer,
                   UINT32 FrameCount, WORD InputChannelCount,
                   WORD OutputChannelCount, BOOL MixWithOutput);
...
public:
  ...
  // decrements reference count and deletes the object if the reference count
  // falls to zero
  ...
  (__deref_out XAPO_REGISTRATION_PROPERTIES **ppRegistrationProperties);
  ...
  (const WAVEFORMATEX *pOutputFormat, const WAVEFORMATEX *pRequestedInputFormat,
   __deref_opt_out WAVEFORMATEX **ppSupportedInputFormat);

  // Queries if a specific output format is supported for a given input format.
  // This default implementation assumes only the format described by the
  // XAPOBASE_DEFAULT_FORMAT values are supported for both input and output.
  STDMETHOD(IsOutputFormatSupported)
  (const WAVEFORMATEX *pInputFormat, const WAVEFORMATEX *pRequestedOutputFormat,
   __deref_opt_out WAVEFORMATEX **ppSupportedOutputFormat);

  // Performs any effect-specific initialization.
  // This default implementation is a no-op and only returns S_OK.
  STDMETHOD(Initialize)
  (__in_bcount_opt(DataByteSize) const void *, UINT32 DataByteSize) {
  ...
  // Returns the number of input frames required to generate the requested
  // number of output frames. By default, this method returns the same number of
  // frames it was passed.
  STDMETHOD_(UINT32, CalcInputFrames)(UINT32 OutputFrameCount) {
    return OutputFrameCount;
  }

  // Returns the number of output frames generated for the requested number of
  // input frames. By default, this method returns the same number of frames it
  // was passed.
...
////
class __declspec(novtable) CXAPOParametersBase : public CXAPOBase,
                                                 public IXAPOParameters {
private:
  BYTE *m_pParameterBlocks; // three contiguous process parameter blocks used
                            // for synchronization, user responsible for
                            // initialization of parameter blocks before
                            // IXAPO::Process/SetParameters/GetParameters called
  BYTE *m_pCurrentParameters; // pointer to current process parameters, must be
                              // aligned for atomic operations
  BYTE *m_pCurrentParametersInternal; // pointer to current process parameters
                                      // (temp pointer read by
                                      // SetParameters/BeginProcess/EndProcess)
  UINT32 m_uCurrentParametersIndex;   // index of current process parameters
  UINT32 m_uParameterBlockByteSize;   // size of a single parameter block in
                                      // bytes, must be > 0
  BOOL m_fNewerResultsReady; // TRUE if there exists new processing results not
                             // yet picked up by GetParameters(), must be
                             // aligned for atomic operations
  BOOL m_fProducer;          // IXAPO::Process produces data to be returned by
                             // GetParameters(); SetParameters() disallowed

public:
  ////
  // PARAMETERS:
  //  pRegistrationProperties - [in] registration properties of the XAPO
  //  pParameterBlocks        - [in] three contiguous process parameter blocks
  //  used for synchronization uParameterBlockByteSize - [in] size of one of the
  //  parameter blocks, must be > 0 fProducer               - [in] TRUE if
  //  IXAPO::Process produces data to be returned by GetParameters()
  //  (SetParameters() and ParametersChanged() disallowed)
  ////
  CXAPOParametersBase(
  ...
  (__in_bcount(ParameterByteSize) const void *pParameters,
  ...
  (__out_bcount(ParameterByteSize) void *pParameters, UINT32 ParameterByteSize);
  ...
  // so the user may assume/assert ParameterByteSize ==
  // m_uParameterBlockByteSize. This method should not block as it is called
  // from the realtime thread.
  virtual void OnSetParameters(const void *, UINT32) {}

  // Returns TRUE if SetParameters() has been called since the last processing
  // pass. May only be used within the XAPO's IXAPO::Process implementation,
