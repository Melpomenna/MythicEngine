...
#define XAUDIO2_MAX_BUFFER_BYTES                                               \
  0x80000000 // Maximum bytes allowed in a source buffer
#define XAUDIO2_MAX_QUEUED_BUFFERS                                             \
  64 // Maximum buffers allowed in a voice queue
#define XAUDIO2_MAX_BUFFERS_SYSTEM                                             \
...
#define XAUDIO2_MAX_VOLUME_LEVEL                                               \
  16777216.0f // Maximum acceptable volume level (2^24)
#define XAUDIO2_MIN_FREQ_RATIO                                                 \
  (1 / 1024.0f)                         // Minimum SetFrequencyRatio argument
...
#define XAUDIO2_MAX_FILTER_ONEOVERQ                                            \
  1.5f // Maximum XAUDIO2_FILTER_PARAMETERS.OneOverQ
#define XAUDIO2_MAX_FILTER_FREQUENCY                                           \
  1.0f // Maximum XAUDIO2_FILTER_PARAMETERS.Frequency
#define XAUDIO2_MAX_LOOP_COUNT                                                 \
  254 // Maximum non-infinite XAUDIO2_BUFFER.LoopCount
#define XAUDIO2_MAX_INSTANCES                                                  \
  8 // Maximum simultaneous XAudio2 objects on Xbox 360

// For XMA voices on Xbox 360 there is an additional restriction on the
// MaxFrequencyRatio argument and the voice's sample rate: the product of these
// numbers cannot exceed 600000 for one-channel voices or 300000 for voices with
...
#define XAUDIO2_INVALID_OPSET                                                  \
  (UINT32)(-1)                       // Not allowed for OperationSet arguments
...
#define XAUDIO2_VOICE_USEFILTER                                                \
  0x0008                           // Used in IXAudio2::CreateSource/SubmixVoice
...
#endif
#define XAUDIO2_QUANTUM_MS                                                     \
  (1000.0f * XAUDIO2_QUANTUM_NUMERATOR / XAUDIO2_QUANTUM_DENOMINATOR)

// XAudio2 error codes
#define FACILITY_XAUDIO2 0x896
#define XAUDIO2_E_INVALID_CALL                                                 \
  0x88960001 // An API call or one of its arguments was illegal
...
// Used in IXAudio2Voice::Set/GetFilterParameters and
// Set/GetOutputFilterParameters
...
typedef struct XAUDIO2_BUFFER_WMA {
  const UINT32
      *pDecodedPacketCumulativeBytes; // Decoded packet's cumulative size array.
                                      //  Each element is the number of bytes
                                      //  accumulated when the corresponding
                                      //  XWMA packet is decoded in order.  The
                                      //  array must have PacketCount elements.
...
typedef struct XAUDIO2_VOICE_STATE {
  void *pCurrentBufferContext; // The pContext value provided in the
                               // XAUDIO2_BUFFER
...
typedef struct XAUDIO2_PERFORMANCE_DATA {
  ...
  UINT64 AudioCyclesSinceLastQuery; // CPU cycles spent on audio processing
                                    // since the
                                    //  last call to StartEngine or
                                    //  GetPerformanceData.
  UINT64
  TotalCyclesSinceLastQuery; // Total CPU cycles elapsed since the last call
                             //  (only counts the CPU XAudio2 is running
                             //  on).
  ...
  UINT32 CurrentLatencyInSamples;    // Minimum delay from when a sample is read
                                     // from a
                                     //  source buffer to when it reaches the
                                     //  speakers.
  UINT32 GlitchesSinceEngineStarted; // Audio dropouts since the engine was
                                     // started.
...
#define XAUDIO2_LOG_WARNINGS                                                   \
  0x0002                        // For handled errors that may be recoverable.
...
DECLARE_INTERFACE_(IXAudio2, IUnknown) {
  ...
  (THIS_ UINT32 Index, __out XAUDIO2_DEVICE_DETAILS * pDeviceDetails) PURE;
  ...
  (THIS_ UINT32 Flags X2DEFAULT(0),
  ...
  (THIS_ __deref_out IXAudio2SourceVoice * *ppSourceVoice,
   __in const WAVEFORMATEX *pSourceFormat, UINT32 Flags X2DEFAULT(0),
  ...
  (THIS_ __deref_out IXAudio2SubmixVoice * *ppSubmixVoice, UINT32 InputChannels,
   UINT32 InputSampleRate, UINT32 Flags X2DEFAULT(0),
  ...
  //
  // ARGUMENTS:
  //  ppMasteringVoice - Returns the new object's IXAudio2MasteringVoice
  //  interface. InputChannels - Number of channels in this voice's input audio
  //  data. InputSampleRate - Sample rate of this voice's input audio data.
...
#define Declare_IXAudio2Voice_Methods()                                        \
  ...
  */                                                                           \
  ...
  (THIS_ UINT32 EffectIndex, __out BOOL * pEnabled) PURE;                      \
                                                                               \
  ...
  */                                                                           \
  ...
  // ARGUMENTS:                                                                \
  //  EffectIndex - Index of an effect within this voice's effect chain.       \
  //  pParameters - Returns the current values of the effect-specific          \
  parameters.                                                                  \
  //  ParametersByteSize - Size of the pParameters array in bytes.             \
  */                                                                           \
  ...
  */                                                                           \
  ...
  //                                                                           \
  // ARGUMENTS:                                                                \
  //  pDestinationVoice - Destination voice of the send whose filter           \
  ...
  */                                                                           \
  ...
  // DESCRIPTION: Returns the filter parameters from one of this voice's       \
  sends.                                                                       \
  //                                                                           \
  // ARGUMENTS:                                                                \
  //  pDestinationVoice - Destination voice of the send whose filter           \
  ...
  */                                                                           \
  STDMETHOD(SetVolume)                                                         \
  (THIS_ float Volume, UINT32 OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW))      \
      PURE;                                                                    \
                                                                               \
  ...
  */                                                                           \
  ...
  (THIS_ UINT32 Channels, __in_ecount(Channels) const float *pVolumes,         \
   UINT32 OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;                    \
                                                                               \
  ...
  */                                                                           \
  ...
  (THIS_ UINT32 Channels, __out_ecount(Channels) float *pVolumes) PURE;        \
                                                                               \
...
DECLARE_INTERFACE_(IXAudio2SourceVoice, IXAudio2Voice) {
  ...
  (THIS_ __in const XAUDIO2_BUFFER *pBuffer,
   __in_opt const XAUDIO2_BUFFER_WMA *pBufferWMA X2DEFAULT(NULL)) PURE;
  ...
  //
  // ARGUMENTS:
  //  Ratio - Frequency change, expressed as source frequency / target
  //  frequency. OperationSet - Used to identify this call as part of a deferred
  //  batch.
  //
  STDMETHOD(SetFrequencyRatio)
  (THIS_ float Ratio, UINT32 OperationSet X2DEFAULT(XAUDIO2_COMMIT_NOW)) PURE;
...
#define IXAudio2_Initialize(This, Flags, XAudio2Processor)                     \
...
#define IXAudio2_CreateSourceVoice(This, ppSourceVoice, pSourceFormat, Flags,  \
  ...
  ((This)->lpVtbl->CreateSourceVoice(This, ppSourceVoice, pSourceFormat,       \
                                     Flags, MaxFrequencyRatio, pCallback,      \
...
#define IXAudio2Voice_SetChannelVolumes(This, Channels, pVolumes,              \
                                        OperationSet)                          \
...
#define IXAudio2SubmixVoice_SetEffectParameters                                \
  IXAudio2Voice_SetEffectParameters
#define IXAudio2SubmixVoice_GetEffectParameters                                \
  IXAudio2Voice_GetEffectParameters
#define IXAudio2SubmixVoice_SetFilterParameters                                \
  IXAudio2Voice_SetFilterParameters
#define IXAudio2SubmixVoice_GetFilterParameters                                \
  IXAudio2Voice_GetFilterParameters
#define IXAudio2SubmixVoice_SetOutputFilterParameters                          \
  IXAudio2Voice_SetOutputFilterParameters
#define IXAudio2SubmixVoice_GetOutputFilterParameters                          \
  IXAudio2Voice_GetOutputFilterParameters
...
__inline float XAudio2CutoffFrequencyToRadians(float CutoffFrequency,
                                               UINT32 SampleRate) {
...
__inline float XAudio2RadiansToCutoffFrequency(float Radians,
                                               float SampleRate) {
...
STDAPI XAudio2Create(
    __deref_out IXAudio2 **ppXAudio2, UINT32 Flags X2DEFAULT(0),
    XAUDIO2_PROCESSOR XAudio2Processor X2DEFAULT(XAUDIO2_DEFAULT_PROCESSOR));

#else // Windows

__inline HRESULT XAudio2Create(
    __deref_out IXAudio2 **ppXAudio2, UINT32 Flags X2DEFAULT(0),
    XAUDIO2_PROCESSOR XAudio2Processor X2DEFAULT(XAUDIO2_DEFAULT_PROCESSOR)) {
  ...
  HRESULT hr = CoCreateInstance(
      (Flags & XAUDIO2_DEBUG_ENGINE) ? __uuidof(XAudio2_Debug)
                                     : __uuidof(XAudio2),
  ...
  HRESULT hr = CoCreateInstance(
      (Flags & XAUDIO2_DEBUG_ENGINE) ? &CLSID_XAudio2_Debug : &CLSID_XAudio2,
