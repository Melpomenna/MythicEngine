...
// scope data, this maps variable names to debug variables (useful for the watch
// window)
typedef struct _D3D10_SHADER_DEBUG_SCOPE_INFO {
  ...
  UINT uVariables;
  UINT VariableData; // Offset to UINT[uVariables] indexing the Scope Variable
                     // list
...
typedef struct _D3D10_SHADER_DEBUG_INST_INFO {
  ...
  // Number of scopes
  UINT Scopes;
  UINT ScopeInfo; // Offset to UINT[uScopes] specifying indices of the ScopeInfo
                  // Array

  // list of variables accessed by this instruction
  // Number of variables
  UINT AccessedVars;
  UINT AccessedVarsInfo; // Offset to UINT[AccessedVars] specifying indices of
                         // the ScopeVariableInfo Array
...
typedef struct _D3D10_SHADER_DEBUG_INFO {
  ...
  UINT InputVariableInfo; // Offset to
                          // D3D10_SHADER_DEBUG_INPUT_INFO[InputVariables]
  ...
  UINT UintOffset;   // Offset to the UINT datastore, all UINT offsets are from
                     // this offset
  UINT StringOffset; // Offset to the string datastore, all string offsets are
                     // from this offset
...
DEFINE_GUID(IID_ID3D10ShaderReflection1, 0xc3457783, 0xa846, 0x47ce, 0x95, 0x20,
            0xce, 0xa6, 0xf6, 0x6e, 0x74, 0x47);
...
DECLARE_INTERFACE_(ID3D10ShaderReflection1, IUnknown) {
  ...
  (THIS_ LPCSTR Name, D3D10_SHADER_INPUT_BIND_DESC * pDesc) PURE;
