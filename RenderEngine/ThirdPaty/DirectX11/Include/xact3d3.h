...
#pragma warning(push)
#pragma warning(disable : 4701) // disable "local variable may be used without
                                // having been initialized" compile warning
...
//  HResult error code
////
EXTERN_C HRESULT inline XACT3DInitialize(__in IXACT3Engine *pEngine,
                                         __in X3DAUDIO_HANDLE X3DInstance) {
  HRESULT hr = S_OK;
  if (pEngine == NULL) {
    hr = E_POINTER;
  }
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
      X3DAudioInitialize(wfxFinalMixFormat.dwChannelMask, nSpeedOfSound,
                         X3DInstance);
    }
  }
  return hr;
...
//
//    X3DAUDIO_DSP_SETTINGS.pMatrixCoefficients, the caller need only allocate
//    space for SrcChannelCount*DstChannelCount elements
//    X3DAUDIO_DSP_SETTINGS.SrcChannelCount
//    X3DAUDIO_DSP_SETTINGS.DstChannelCount
//
//  If X3DAUDIO_EMITTER.pChannelAzimuths is left NULL for multi-channel
//  emitters, a default channel radius and channel azimuth array will be applied
//  below. Distance curves such as X3DAUDIO_EMITTER.pVolumeCurve should be left
//  NULL as XACT's native RPCs will be used to define DSP behaviour with respect
//  to normalized distance.
//
//  See X3DAudio.h for information regarding X3DAudio types.
//
// PARAMETERS:
//  X3DInstance  - [in]  X3DAudio instance handle, returned from
//  XACT3DInitialize() pListener    - [in]  point of 3D audio reception pEmitter
//  - [in]  3D audio source pDSPSettings - [out] receives calculation results,
//  applied to an XACT cue via XACT3DApply()
//
// RETURN VALUE:
//  HResult error code
////
EXTERN_C HRESULT inline XACT3DCalculate(
    __in X3DAUDIO_HANDLE X3DInstance, __in const X3DAUDIO_LISTENER *pListener,
    __inout X3DAUDIO_EMITTER *pEmitter,
...
////
EXTERN_C
HRESULT inline XACT3DApply(__in const X3DAUDIO_DSP_SETTINGS *pDSPSettings,
                           __in IXACT3Cue *pCue) {
  HRESULT hr = S_OK;
