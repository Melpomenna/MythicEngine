...
DEFINE_GUID(IID_ID3DX11Scan, 0x5089b68f, 0xe71d, 0x4d38, 0xbe, 0x8e, 0xf3, 0x63,
            0xb9, 0x5a, 0x94, 0x05);
...
DECLARE_INTERFACE_(ID3DX11Scan, IUnknown) {
  ...
  //  pSrc                input sequence on the device. pSrc==pDst for in-place
  //  scans pDst                output sequence on the device
  //=============================================================================
  STDMETHOD(Scan)
  (THIS_ D3DX11_SCAN_DATA_TYPE ElementType, D3DX11_SCAN_OPCODE OpCode,
   UINT ElementScanSize, __in ID3D11UnorderedAccessView * pSrc,
   __in ID3D11UnorderedAccessView * pDst) PURE;
  ...
  STDMETHOD(Multiscan)
  (THIS_ D3DX11_SCAN_DATA_TYPE ElementType, D3DX11_SCAN_OPCODE OpCode,
   UINT ElementScanSize, UINT ElementScanPitch, UINT ScanCount,
   __in ID3D11UnorderedAccessView * pSrc, __in ID3D11UnorderedAccessView * pDst)
      PURE;
};
...
//  MaxElementScanSize  maximum single scan size, in elements (FLOAT, UINT, or
//  INT) MaxScanCount        maximum number of scans in multiscan ppScanContext
//  new scan context
//=============================================================================
HRESULT WINAPI D3DX11CreateScan(__in ID3D11DeviceContext *pDeviceContext,
                                UINT MaxElementScanSize, UINT MaxScanCount,
...
DEFINE_GUID(IID_ID3DX11SegmentedScan, 0xa915128c, 0xd954, 0x4c79, 0xbf, 0xe1,
            0x64, 0xdb, 0x92, 0x31, 0x94, 0xd6);
...
DECLARE_INTERFACE_(ID3DX11SegmentedScan, IUnknown) {
  ...
  //  OpCode              binary operation
  //  Direction           scan direction
  //  pSrcElementFlags    compact array of bits, one per element of pSrc.  A set
  //  value
  ...
  STDMETHOD(SegScan)
  (THIS_ D3DX11_SCAN_DATA_TYPE ElementType, D3DX11_SCAN_OPCODE OpCode,
   UINT ElementScanSize, __in_opt ID3D11UnorderedAccessView * pSrc,
...
//  pDevice             the device context
//  MaxElementScanSize  maximum single scan size, in elements (FLOAT, UINT, or
//  INT) ppScanContext       new scan context
//=============================================================================
HRESULT WINAPI D3DX11CreateSegmentedScan(
    __in ID3D11DeviceContext *pDeviceContext, UINT MaxElementScanSize,
...
DEFINE_GUID(IID_ID3DX11FFT, 0xb3f7a938, 0x4c93, 0x4310, 0xa6, 0x75, 0xb3, 0x0d,
            0x6d, 0xe5, 0x05, 0x53);
...
DECLARE_INTERFACE_(ID3DX11FFT, IUnknown) {
  ...
  // though care should be taken to concurrently execute multiple FFTs which
  // share temp buffers.
  //
  ...
  STDMETHOD(AttachBuffersAndPrecompute)
  (THIS_ __in_range(0, D3DX11_FFT_MAX_TEMP_BUFFERS) UINT NumTempBuffers,
   ...
   __in_ecount(NumPrecomputeBuffers)
       ID3D11UnorderedAccessView *const *ppPrecomputeBufferSizes) PURE;

  //------------------------------------------------------------------------------
  // Call after buffers have been attached to the context, pInput and *ppOuput
  // can be one of the temp buffers.  If *ppOutput == NULL, then the computation
  // will ping-pong between temp buffers and the last buffer written to is
  // stored at *ppOutput. Otherwise, *ppOutput is used as the output buffer
  // (which may incur an extra copy).
  //
  ...
  STDMETHOD(ForwardTransform)
  (THIS_ __in const ID3D11UnorderedAccessView *pInputBuffer,
...
typedef enum D3DX11_FFT_CREATE_FLAG {
  D3DX11_FFT_CREATE_FLAG_NO_PRECOMPUTE_BUFFERS =
      0x01L, // do not precompute values and store into buffers
} D3DX11_FFT_CREATE_FLAG;

//------------------------------------------------------------------------------
// Creates an ID3DX11FFT COM interface object and returns a pointer to it at
// *ppFFT. The descriptor describes the shape of the data as well as the scaling
// factors that should be used for forward and inverse transforms. The FFT
// computation may require temporaries that act as ping-pong buffers and for
// other purposes. aTempSizes is a list of the sizes required for temporaries.
// Likewise, some data may need to be precomputed and the sizes of those sizes
// are returned in aPrecomputedBufferSizes.
//
...
//
// Device                     Direct3DDeviceContext to use in pDesc Descriptor
// for FFT transform                      in Count                      the
// number of 1D FFTs to perform                  in Flags See
// D3DX11_FFT_CREATE_FLAG                        in pBufferInfo Pointer to
// BUFFER_INFO struct, filled by funciton out ppFFT                      Pointer
// to returned context pointer               out
//------------------------------------------------------------------------------

HRESULT WINAPI D3DX11CreateFFT(ID3D11DeviceContext *pDeviceContext,
                               __in const D3DX11_FFT_DESC *pDesc, UINT Flags,
...
HRESULT WINAPI D3DX11CreateFFT1DComplex(
    ID3D11DeviceContext *pDeviceContext, UINT X, UINT Flags,
    __out D3DX11_FFT_BUFFER_INFO *pBufferInfo, __out ID3DX11FFT **ppFFT);
HRESULT WINAPI D3DX11CreateFFT2DReal(ID3D11DeviceContext *pDeviceContext,
                                     UINT X, UINT Y, UINT Flags,
                                     __out D3DX11_FFT_BUFFER_INFO *pBufferInfo,
                                     __out ID3DX11FFT **ppFFT);
HRESULT WINAPI D3DX11CreateFFT2DComplex(
    ID3D11DeviceContext *pDeviceContext, UINT X, UINT Y, UINT Flags,
    __out D3DX11_FFT_BUFFER_INFO *pBufferInfo, __out ID3DX11FFT **ppFFT);
HRESULT WINAPI D3DX11CreateFFT3DReal(ID3D11DeviceContext *pDeviceContext,
                                     UINT X, UINT Y, UINT Z, UINT Flags,
                                     __out D3DX11_FFT_BUFFER_INFO *pBufferInfo,
                                     __out ID3DX11FFT **ppFFT);
HRESULT WINAPI D3DX11CreateFFT3DComplex(
    ID3D11DeviceContext *pDeviceContext, UINT X, UINT Y, UINT Z, UINT Flags,
    __out D3DX11_FFT_BUFFER_INFO *pBufferInfo, __out ID3DX11FFT **ppFFT);
