...
//
// The encoded XMA2 data is structured as a set of BLOCKS, which contain
// PACKETS, which contain FRAMES, which contain SUBFRAMES (roughly speaking).
// The frames in a file may also be divided into several subsets, called
// STREAMS.
//
...
//      samples.  The XMA decoder cannot decode a subframe in isolation; it
//      needs a whole frame to work with.  However, it can begin emitting the
//      frame's decoded samples at any one of the four subframe boundaries.
//      Subframes can be addressed for seeking and looping purposes.
//
// PACKET: A 2Kb region containing a 32-bit header and some XMA frames.  Frames
//      can (and usually do) span packets.  A packet's header includes the
//      offset in bits of the first frame that begins within that packet.  All
//      of the frames that begin in a given packet belong to the same "stream"
//      (see the Multichannel Audio section below).
//
...
//      4095 packets (8190Kb), but its size is typically in the 32Kb-128Kb
//      range. (The size chosen involves a trade-off between memory use and
//      efficiency of reading from permanent storage.)
//
...
// order.  To locate a frame that belongs to a given stream in a given XMA
// block, you must examine the first few packets in the block.  Here (and only
// here) the packets are guaranteed to be presented in stream order, so that all
// frames beginning in packet 0 belong to stream 0 (the first stereo pair), etc.
//
...
#define XMA_READBUFFER_MAX_BYTES                                               \
  (XMA_READBUFFER_MAX_PACKETS * XMA_BYTES_PER_PACKET)
...
typedef struct XMASTREAMFORMAT {
  DWORD PsuedoBytesPerSec; // Used by the XMA encoder (typo preserved for legacy
                           // reasons)
...
typedef struct XMA2PACKET {
  ...
  int FrameOffsetInBits : 15; // Bit of XmaData where the first complete frame
                              // begins
  int PacketMetaData : 3;  // Metadata stored in the packet (always 1 for XMA2)
  int PacketSkipCount : 8; // How many packets belonging to other streams must
                           // be skipped to find the next packet belonging to
                           // this one
...
//    |          |         |      |____ Skip 2 packets to find the next one for
//    this stream |          |         |___________ XMA2 signature (always 001)
...
__inline DWORD GetXmaPacketFirstFrameOffsetInBits(__in_bcount(3)
                                                      const BYTE *pPacket) {
  return ((DWORD)(pPacket[0] & 0x3) << 13) | ((DWORD)(pPacket[1]) << 5) |
         ((DWORD)(pPacket[2]) >> 3);
}
...
__inline HRESULT GetXmaBlockContainingSample(
    ...
    __in_ecount(nBlockCount)
        const DWORD *pSeekTable, // Pointer to the seek table data
    DWORD nDesiredSample,        // Decoded sample to locate
    __out DWORD
        *pnBlockContainingSample, // Index of the block containing the sample
    __out DWORD
        *pnSampleOffsetWithinBlock // Position of the sample in this block
) {
...
__inline DWORD GetXmaFrameLengthInBits(
    __in_bcount(nBitPosition / 8 + 3) __in const BYTE
        *pPacket,      // Pointer to XMA packet[s] containing the frame
  ...
  if (nBitOffset <
      2) // Only need to read 2 bytes (and might not be safe to read more)
  {
...
__inline DWORD
GetXmaFrameBitPosition(__in_bcount(nXmaDataBytes)
  ...
  for (;;) {
    ...
    if (nFrameCountSoFar + GetXmaPacketFrameCount(pCurrentPacket) >
        nDesiredFrame) {
      ...
      nFrameBitOffset = XMA_PACKET_HEADER_BITS +
                        GetXmaPacketFirstFrameOffsetInBits(pCurrentPacket);

      // Advance nFrameBitOffset to the frame of interest
      while (nFramesToSkip--) {
        nFrameBitOffset +=
            GetXmaFrameLengthInBits(pCurrentPacket, nFrameBitOffset);
      }
    ...
    pCurrentPacket +=
        XMA_BYTES_PER_PACKET * (GetXmaPacketSkipCount(pCurrentPacket) + 1);
  }
}
...
__inline DWORD
GetLastXmaFrameBitPosition(__in_bcount(nXmaDataBytes)
                               const BYTE *pXmaData, // Pointer to XMA block[s]
                           DWORD nXmaDataBytes, // Size of pXmaData in bytes
                           DWORD nStreamIndex   // Stream within which to seek
) {
  ...
  for (;;) {
    nBytesToNextPacket =
        XMA_BYTES_PER_PACKET * (GetXmaPacketSkipCount(pLastPacket) + 1);
    XMA2DEFS_ASSERT(nBytesToNextPacket);
    if (pLastPacket + nBytesToNextPacket + XMA_BYTES_PER_PACKET >
        pXmaData + nXmaDataBytes) {
  ...
  nFrameBitOffset =
      XMA_PACKET_HEADER_BITS + GetXmaPacketFirstFrameOffsetInBits(pLastPacket);
...
__inline HRESULT GetXmaDecodePositionForSample(
  ...
  DWORD nSubFrame =
      (nDesiredSample % XMA_SAMPLES_PER_FRAME) / XMA_SAMPLES_PER_SUBFRAME;
  DWORD nBitOffset = GetXmaFrameBitPosition(pXmaData, nXmaDataBytes,
                                            nStreamIndex, nDesiredFrame);
