...
D2D1FORCEINLINE
D2D1_SIZE_F
SizeF(FLOAT width = 0.f, FLOAT height = 0.f) {
  return Size<FLOAT>(width, height);
}

D2D1FORCEINLINE
D2D1_SIZE_U
SizeU(UINT32 width = 0, UINT32 height = 0) {
  return Size<UINT32>(width, height);
}

template <typename Type>
D2D1FORCEINLINE typename TypeTraits<Type>::Rect Rect(Type left, Type top,
...
D2D1FORCEINLINE
D2D1_RECT_F
RectF(FLOAT left = 0.f, FLOAT top = 0.f, FLOAT right = 0.f,
      FLOAT bottom = 0.f) {
...
D2D1FORCEINLINE
D2D1_ARC_SEGMENT
ArcSegment(__in CONST D2D1_POINT_2F &point, __in CONST D2D1_SIZE_F &size,
           __in FLOAT rotationAngle, __in D2D1_SWEEP_DIRECTION sweepDirection,
           __in D2D1_ARC_SIZE arcSize) {
  D2D1_ARC_SEGMENT arcSegment = {point, size, rotationAngle, sweepDirection,
                                 arcSize};
...
D2D1FORCEINLINE
D2D1_BEZIER_SEGMENT
BezierSegment(__in CONST D2D1_POINT_2F &point1,
...
D2D1FORCEINLINE
D2D1_ELLIPSE
Ellipse(__in CONST D2D1_POINT_2F &center, FLOAT radiusX, FLOAT radiusY) {
...
D2D1FORCEINLINE
D2D1_GRADIENT_STOP
GradientStop(FLOAT position, __in CONST D2D1_COLOR_F &color) {
...
QuadraticBezierSegment(__in CONST D2D1_POINT_2F &point1,
                       __in CONST D2D1_POINT_2F &point2) {
...
StrokeStyleProperties(D2D1_CAP_STYLE startCap = D2D1_CAP_STYLE_FLAT,
...
BitmapBrushProperties(D2D1_EXTEND_MODE extendModeX = D2D1_EXTEND_MODE_CLAMP,
...
LinearGradientBrushProperties(__in CONST D2D1_POINT_2F &startPoint,
...
RadialGradientBrushProperties(__in CONST D2D1_POINT_2F &center,
...
D2D1FORCEINLINE
D2D1_PIXEL_FORMAT
PixelFormat(__in DXGI_FORMAT dxgiFormat = DXGI_FORMAT_UNKNOWN,
...
BitmapProperties(CONST D2D1_PIXEL_FORMAT &pixelFormat = D2D1::PixelFormat(),
...
HwndRenderTargetProperties(
    __in HWND hwnd,
    __in D2D1_SIZE_U pixelSize = D2D1::Size(static_cast<UINT>(0),
                                            static_cast<UINT>(0)),
...
LayerParameters(
    ...
    FLOAT opacity = 1.0, __in_opt ID2D1Brush *opacityBrush = NULL,
...
DrawingStateDescription(
    ...
    D2D1_TAG tag1 = 0, D2D1_TAG tag2 = 0,
    __in const D2D1_MATRIX_3X2_F &transform = D2D1::IdentityMatrix()) {
...
public:
  ...
  D2D1FORCEINLINE
  ColorF(FLOAT r, FLOAT g, FLOAT b, FLOAT a = 1.0) {
    this->r = r;
    this->g = g;
    this->b = b;
    this->a = a;
...
public:
  ...
  static D2D1FORCEINLINE Matrix3x2F
  Scale(D2D1_SIZE_F size, D2D1_POINT_2F center = D2D1::Point2F()) {
    Matrix3x2F scale;
  ...
  static D2D1FORCEINLINE Matrix3x2F
  Scale(FLOAT x, FLOAT y, D2D1_POINT_2F center = D2D1::Point2F()) {
    return Scale(SizeF(x, y), center);
  }

  static D2D1FORCEINLINE Matrix3x2F
  Rotation(FLOAT angle, D2D1_POINT_2F center = D2D1::Point2F()) {
    Matrix3x2F rotation;
  ...
  static D2D1FORCEINLINE Matrix3x2F
  Skew(FLOAT angleX, FLOAT angleY, D2D1_POINT_2F center = D2D1::Point2F()) {
    Matrix3x2F skew;
  ...
  //
  static inline const Matrix3x2F *
  ...
  TransformPoint(D2D1_POINT_2F point) const {
    D2D1_POINT_2F result = {point.x * _11 + point.y * _21 + _31,
...
D2D1FORCEINLINE
D2D1_POINT_2F
operator*(const D2D1_POINT_2F & point, const D2D1_MATRIX_3X2_F & matrix) {
...
D2D1FORCEINLINE
D2D1_MATRIX_3X2_F
operator*(const D2D1_MATRIX_3X2_F & matrix1,
          const D2D1_MATRIX_3X2_F & matrix2) {
  return (*D2D1::Matrix3x2F::ReinterpretBaseType(&matrix1)) *
