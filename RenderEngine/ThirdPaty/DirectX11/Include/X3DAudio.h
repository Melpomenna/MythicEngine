...
#define SPEAKER_RESERVED                                                       \
  0x7FFC0000 // bit mask locations reserved for future use
#define SPEAKER_ALL                                                            \
  0x80000000 // used to specify that any possible permutation of speaker
             // configurations
#endif
...
#define X3DAUDIO_CALCULATE_ZEROCENTER                                          \
  0x00010000 // do not position to front center speaker, signal positioned to
             // remaining speakers instead, front center destination channel
             // will be zero in returned matrix coefficient table, valid only
             // for matrix calculations with final mix formats that have a front
             // center channel
#define X3DAUDIO_CALCULATE_REDIRECT_TO_LFE                                     \
  0x00020000 // apply equal mix of all source channels to LFE destination
             // channel, valid only for matrix calculations with sources that
             // have no LFE channel and final mix formats that have an LFE
             // channel
...
typedef struct X3DAUDIO_CONE {
  ...
  OuterVolume; // volume level scaler on/beyond outer cone, used only for
               // matrix calculations, must be within [0.0f, 2.0f] when used
  FLOAT32 InnerLPF; // LPF (both direct and reverb paths) coefficient subtrahend
                    // on/within inner cone, used only for LPF (both direct and
                    // reverb paths) calculations, must be within [0.0f, 1.0f]
                    // when used
  FLOAT32 OuterLPF; // LPF (both direct and reverb paths) coefficient subtrahend
...
static const X3DAUDIO_CONE X3DAudioDefault_DirectionalCone = {
    X3DAUDIO_PI / 2, X3DAUDIO_PI, 1.0f, 0.708f, 0.0f, 0.25f, 0.708f, 1.0f};
...
typedef struct X3DAUDIO_LISTENER {
  X3DAUDIO_VECTOR
  OrientFront; // orientation of front direction, used only for matrix and
               // delay calculations or listeners with cones for matrix, LPF
               // (both direct and reverb paths), and reverb calculations,
  ...
  X3DAUDIO_CONE *pCone; // sound cone, used only for matrix, LPF (both direct
                        // and reverb paths), and reverb calculations, NULL
                        // specifies omnidirectionality
...
typedef struct X3DAUDIO_EMITTER {
  ...
  *pCone; // sound cone, used only with single-channel emitters for matrix,
  ...
  FLOAT32 InnerRadiusAngle; // inner radius angle, must be within [0.0f,
                            // X3DAUDIO_PI/4.0)
  ...
  *pChannelAzimuths; // channel azimuth array, used only with multi-channel
                     ...
                     // orientation vector, or X3DAUDIO_2PI to specify an
                     // LFE channel, must have at least ChannelCount
                     // elements, all within [0.0f, X3DAUDIO_2PI] when used
  ...
  *pVolumeCurve; // volume level distance curve, used only for matrix
                 ...
                 // conforms to the inverse square law, calculated in
                 // user-defined world units with distances <=
                 // CurveDistanceScaler clamped to no attenuation
  X3DAUDIO_DISTANCE_CURVE *
      pLFECurve; // LFE level distance curve, used only for matrix calculations,
                 ...
                 // square law, calculated in user-defined world units with
  ...
  *pReverbCurve; // reverb send level distance curve, used only for reverb
  ...
  FLOAT32 CurveDistanceScaler; // curve distance scaler, used to scale
                               ...
                               // used only for matrix, LPF (both direct and
                               // reverb paths), and reverb calculations, must
                               // be within [FLT_MIN, FLT_MAX] when used
  FLOAT32 DopplerScaler; // doppler shift scaler, used to exaggerate doppler
                         // shift effect, used only for doppler calculations,
                         // must be within [0.0f, FLT_MAX] when used
...
typedef struct X3DAUDIO_DSP_SETTINGS {
  FLOAT32
  *pMatrixCoefficients; // [inout] matrix coefficient table, receives an
                        // array representing the volume level used to send
                        // from source channel S to destination channel D,
                        // stored as pMatrixCoefficients[SrcChannelCount * D
                        // + S], must have at least
                        // SrcChannelCount*DstChannelCount elements
  FLOAT32
  *pDelayTimes; // [inout] delay time array, receives delays for each
  ...
  UINT32 SrcChannelCount; // [in] number of source channels, must equal number
                          // of channels in respective emitter
  UINT32 DstChannelCount; // [in] number of destination channels, must equal
                          // number of channels of the final mix
...
X3DAudioInitialize(UINT32 SpeakerChannelMask, FLOAT32 SpeedOfSound,
                   __out X3DAUDIO_HANDLE Instance);

// calculates DSP settings with respect to 3D parameters
X3DAUDIO_API_(void)
X3DAudioCalculate(__in const X3DAUDIO_HANDLE Instance,
                  __in const X3DAUDIO_LISTENER *pListener,
                  __in const X3DAUDIO_EMITTER *pEmitter, UINT32 Flags,
                  __inout X3DAUDIO_DSP_SETTINGS *pDSPSettings);
