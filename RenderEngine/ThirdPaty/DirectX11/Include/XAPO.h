...
DEFINE_IID(IXAPOParameters, A90BC001, E897, E897, 55, E4, 9E, 47, 00, 00, 00,
           01);
...
#define XAPO_E_FORMAT_UNSUPPORTED                                              \
  MAKE_HRESULT(SEVERITY_ERROR, FACILITY_XAPO,                                  \
               0x01) // requested audio format unsupported
...
// unicode string length, including terminator, used with
...
//--------------<D-A-T-A---T-Y-P-E-S>---------------------------------------//
#pragma pack(push, 1) // set packing alignment to ensure consistency across
                      // arbitrary build environments

// XAPO registration properties, describes general XAPO characteristics, used
// with IXAPO::GetRegistrationProperties
typedef struct XAPO_REGISTRATION_PROPERTIES {
  CLSID clsid; // COM class ID, used with CoCreate
  WCHAR FriendlyName[XAPO_REGISTRATION_STRING_LENGTH];  // friendly name unicode
                                                        // string
  WCHAR CopyrightInfo[XAPO_REGISTRATION_STRING_LENGTH]; // copyright information
                                                        // unicode string
  UINT32 MajorVersion;                                  // major version
  UINT32 MinorVersion;                                  // minor version
  UINT32 Flags; // XAPO property flags, describes supported input/output
                // configuration
  UINT32 MinInputBufferCount;  // minimum number of input buffers required for
                               // processing, can be 0
  UINT32 MaxInputBufferCount;  // maximum number of input buffers supported for
                               // processing, must be >= MinInputBufferCount
  UINT32 MinOutputBufferCount; // minimum number of output buffers required for
                               // processing, can be 0, must match
                               // MinInputBufferCount when
                               // XAPO_FLAG_BUFFERCOUNT_MUST_MATCH used
  UINT32 MaxOutputBufferCount; // maximum number of output buffers supported for
                               // processing, must be >= MinOutputBufferCount,
                               // must match MaxInputBufferCount when
                               // XAPO_FLAG_BUFFERCOUNT_MUST_MATCH used
} XAPO_REGISTRATION_PROPERTIES;
...
typedef struct XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS {
  ...
  UINT32 MaxFrameCount; // maximum number of frames in respective buffer that
                        // IXAPO::Process would have to handle, irrespective of
                        // dynamic variable settings, can be 0
...
typedef enum XAPO_BUFFER_FLAGS {
  XAPO_BUFFER_SILENT, // silent data should be assumed, respective memory may be
                      // uninitialized
  XAPO_BUFFER_VALID,  // arbitrary data should be assumed (may or may not be
                      // silent frames), respective memory initialized
} XAPO_BUFFER_FLAGS;
...
//      XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.pFormat->nBlockAlign
//
...
typedef struct XAPO_PROCESS_BUFFER_PARAMETERS {
  ...
  XAPO_BUFFER_FLAGS BufferFlags; // describes assumed content of pBuffer, does
                                 // not affect ValidFrameCount
  UINT32
  ValidFrameCount; // number of frames of valid data, must be within
                   // respective [0,
                   // XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount],
                   // always
                   // XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount
                   // for CBR/user-defined XAPOs, does not affect
                   // BufferFlags
...
#if defined(_XBOX)
#define XAPO_ALLOC_ATTRIBUTES                                                  \
  MAKE_XALLOC_ATTRIBUTES(0,     /* ObjectType */                               \
...
DECLARE_INTERFACE_(IXAPO, IUnknown) {
  ...
  //
  // PARAMETERS:
  //  ppRegistrationProperties - [out] receives pointer to copy of registration
  //  properties, use XAPOFree to free structure, left untouched on failure
  //
  // RETURN VALUE:
  //  COM error code
  ////
  STDMETHOD(GetRegistrationProperties)
  (THIS_ __deref_out XAPO_REGISTRATION_PROPERTIES * *ppRegistrationProperties)
      PURE;
  ...
  //  ppSupportedInputFormat - [out] receives pointer to nearest input format
  //  supported if not NULL and input/output configuration unsupported, use
  //  XAPOFree to free structure, left untouched on any failure except
  //  XAPO_E_FORMAT_UNSUPPORTED
  //
  // RETURN VALUE:
  //  COM error code, including:
  //    S_OK                      - input/output configuration supported,
  //    ppSupportedInputFormat left untouched XAPO_E_FORMAT_UNSUPPORTED -
  //    input/output configuration unsupported, ppSupportedInputFormat receives
  //    pointer to nearest input format supported if not NULL E_INVALIDARG -
  //    either audio format invalid, ppSupportedInputFormat left untouched
  ////
  STDMETHOD(IsInputFormatSupported)
  (THIS_ const WAVEFORMATEX *pOutputFormat,
   const WAVEFORMATEX *pRequestedInputFormat,
   __deref_opt_out WAVEFORMATEX **ppSupportedInputFormat) PURE;
  ...
  //  ppSupportedOutputFormat - [out] receives pointer to nearest output format
  //  supported if not NULL and input/output configuration unsupported, use
  //  XAPOFree to free structure, left untouched on any failure except
  //  XAPO_E_FORMAT_UNSUPPORTED
  //
  ...
  //  COM error code, including:
  //    S_OK                      - input/output configuration supported,
  //    ppSupportedOutputFormat left untouched XAPO_E_FORMAT_UNSUPPORTED -
  //    input/output configuration unsupported, ppSupportedOutputFormat receives
  //    pointer to nearest output format supported if not NULL E_INVALIDARG -
  //    either audio format invalid, ppSupportedOutputFormat left untouched
  ////
  STDMETHOD(IsOutputFormatSupported)
  (THIS_ const WAVEFORMATEX *pInputFormat,
   const WAVEFORMATEX *pRequestedOutputFormat,
   __deref_opt_out WAVEFORMATEX **ppSupportedOutputFormat) PURE;
  ...
  //
  // PARAMETERS:
  //  pData        - [in] effect-specific initialization parameters, may be NULL
  //  if DataByteSize == 0 DataByteSize - [in] size of pData in bytes, may be 0
  //  if DataByteSize is NULL
  //
  // RETURN VALUE:
  //  COM error code
  ////
  STDMETHOD(Initialize)
  (THIS_ __in_bcount_opt(DataByteSize) const void *pData, UINT32 DataByteSize)
      PURE;
  ...
  //
  // PARAMETERS:
  //  InputLockedParameterCount  - [in] number of input buffers, must be within
  //  [XAPO_REGISTRATION_PROPERTIES.MinInputBufferCount,
  //  XAPO_REGISTRATION_PROPERTIES.MaxInputBufferCount] pInputLockedParameters
  //  - [in] array of input locked buffer parameter structures, may be NULL if
  //  InputLockedParameterCount == 0, otherwise must have
  //  InputLockedParameterCount elements OutputLockedParameterCount - [in]
  //  number of output buffers, must be within
  //  [XAPO_REGISTRATION_PROPERTIES.MinOutputBufferCount,
  //  XAPO_REGISTRATION_PROPERTIES.MaxOutputBufferCount], must match
  //  InputLockedParameterCount when XAPO_FLAG_BUFFERCOUNT_MUST_MATCH used
  //  pOutputLockedParameters    - [in] array of output locked buffer parameter
  //  structures, may be NULL if OutputLockedParameterCount == 0, otherwise must
  //  have OutputLockedParameterCount elements
  //
  // RETURN VALUE:
  //  COM error code
  ////
  STDMETHOD(LockForProcess)
  (THIS_ UINT32 InputLockedParameterCount,
   __in_ecount_opt(InputLockedParameterCount)
       const XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS *pInputLockedParameters,
   UINT32 OutputLockedParameterCount,
   __in_ecount_opt(OutputLockedParameterCount)
  ...
  //
  // PARAMETERS:
  //  InputProcessParameterCount  - [in]     number of input buffers, matches
  //  respective InputLockedParameterCount parameter given to LockForProcess
  //  pInputProcessParameters     - [in]     array of input process buffer
  //  parameter structures, may be NULL if InputProcessParameterCount == 0,
  //  otherwise must have InputProcessParameterCount elements
  //  OutputProcessParameterCount - [in]     number of output buffers, matches
  //  respective OutputLockedParameterCount parameter given to LockForProcess
  //  pOutputProcessParameters    - [in/out] array of output process buffer
  //  parameter structures, may be NULL if OutputProcessParameterCount == 0,
  //  otherwise must have OutputProcessParameterCount elements IsEnabled - [in]
  //  TRUE to process normally, FALSE to process thru
  //
  // RETURN VALUE:
  //  void
  ////
  STDMETHOD_(void, Process)
  (THIS_ UINT32 InputProcessParameterCount,
   __in_ecount_opt(InputProcessParameterCount)
       const XAPO_PROCESS_BUFFER_PARAMETERS *pInputProcessParameters,
   UINT32 OutputProcessParameterCount,
   __inout_ecount_opt(OutputProcessParameterCount)
  ...
  //
  // PARAMETERS:
  //  OutputFrameCount - [in] requested number of output frames, must be within
  //  respective [0, XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount],
  //  always XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount for
  //  CBR/user-defined XAPOs
  //
  // RETURN VALUE:
  ...
  //
  // PARAMETERS:
  //  InputFrameCount - [in] requested number of input frames, must be within
  //  respective [0, XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount],
  //  always XAPO_LOCKFORPROCESS_BUFFER_PARAMETERS.MaxFrameCount for
  //  CBR/user-defined XAPOs
  //
...
DECLARE_INTERFACE_(IXAPOParameters, IUnknown) {
  ...
  (THIS_ __in_bcount(ParameterByteSize) const void *pParameters,
  ...
  //
  // PARAMETERS:
  //  pParameters       - [out] receives effect-specific parameter block, must
  //  be != NULL ParameterByteSize - [in]  size of pParameters in bytes, must be
  //  > 0
  //
  // RETURN VALUE:
  //  void
  ...
  (THIS_ __out_bcount(ParameterByteSize) void *pParameters,
   UINT32 ParameterByteSize) PURE;
};
...
#define IXAPO_IsInputFormatSupported(                                          \
  ...
  ((This)->lpVtbl->IsInputFormatSupported(                                     \
      This, pOutputFormat, pRequestedInputFormat, ppSupportedInputFormat))

#define IXAPO_IsOutputFormatSupported(                                         \
...
#define IXAPO_LockForProcess(                                                  \
    This, InputLockedParameterCount, pInputLockedParameters,                   \
    OutputLockedParameterCount, pOutputLockedParameters)                       \
  ((This)->lpVtbl->LockForProcess(                                             \
      This, InputLockedParameterCount, pInputLockedParameters,                 \
      OutputLockedParameterCount, pOutputLockedParameters))

#define IXAPO_UnlockForProcess(This) ((This)->lpVtbl->UnlockForProcess(This))

#define IXAPO_Process(This, InputProcessParameterCount,                        \
                      pInputProcessParameters, OutputProcessParameterCount,    \
                      pOutputProcessParameters, IsEnabled)                     \
  ((This)->lpVtbl->Process(                                                    \
      This, InputProcessParameterCount, pInputProcessParameters,               \
      OutputProcessParameterCount, pOutputProcessParameters, IsEnabled))
