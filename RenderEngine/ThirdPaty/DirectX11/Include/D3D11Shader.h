...
typedef struct _D3D11_SIGNATURE_PARAMETER_DESC {
  ...
  D3D_NAME SystemValueType; // A predefined system value, or D3D_NAME_UNDEFINED
                            // if not applicable
...
typedef interface ID3D11ShaderReflectionVariable
    *LPD3D11SHADERREFLECTIONVARIABLE;

typedef interface ID3D11ShaderReflectionConstantBuffer
...
DEFINE_GUID(IID_ID3D11ShaderReflectionType, 0x6e6ffa6a, 0x9bae, 0x4613, 0xa5,
            0x1e, 0x91, 0x65, 0x2d, 0x50, 0x8c, 0x21);
...
DECLARE_INTERFACE(ID3D11ShaderReflectionType) {
  ...
  (THIS_ __in UINT uIndex) PURE;
  STDMETHOD(IsOfType)(THIS_ __in ID3D11ShaderReflectionType * pType) PURE;
  STDMETHOD(ImplementsInterface)
  (THIS_ __in ID3D11ShaderReflectionType * pBase) PURE;
};

// {51F23923-F3E5-4BD1-91CB-606177D8DB4C}
DEFINE_GUID(IID_ID3D11ShaderReflectionVariable, 0x51f23923, 0xf3e5, 0x4bd1,
...
DEFINE_GUID(IID_ID3D11ShaderReflectionConstantBuffer, 0xeb62d63d, 0x93dd,
            0x4318, 0x8a, 0xe8, 0xc6, 0xf8, 0x3a, 0xd3, 0x71, 0xb8);
...
DEFINE_GUID(IID_ID3D11ShaderReflection, 0x0a233719, 0x3960, 0x4578, 0x9d, 0x7c,
            0x20, 0x3b, 0x8b, 0x1d, 0x9c, 0xc1);

#undef INTERFACE
#define INTERFACE ID3D11ShaderReflection

DECLARE_INTERFACE_(ID3D11ShaderReflection, IUnknown) {
  STDMETHOD(QueryInterface)(THIS_ __in REFIID iid, __out LPVOID * ppv) PURE;
  ...
  (THIS_ __in UINT ResourceIndex, __out D3D11_SHADER_INPUT_BIND_DESC * pDesc)
      PURE;

  STDMETHOD(GetInputParameterDesc)
  (THIS_ __in UINT ParameterIndex, __out D3D11_SIGNATURE_PARAMETER_DESC * pDesc)
      PURE;
  ...
  (THIS_ __in LPCSTR Name, __out D3D11_SHADER_INPUT_BIND_DESC * pDesc) PURE;
  ...
  (THIS_ __out enum D3D_FEATURE_LEVEL * pLevel) PURE;

  STDMETHOD_(UINT, GetThreadGroupSize)
  (THIS_ __out_opt UINT * pSizeX, __out_opt UINT * pSizeY,
